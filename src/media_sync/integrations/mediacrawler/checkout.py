...
def _mapping(value: object, name: str) -> Mapping[str, object]:
    if not isinstance(value, Mapping):
        raise CheckoutValidationError(f"{name} must be a JSON object")
    return {str(key): item for key, item in value.items()}


def _required_text(value: object, name: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise CheckoutValidationError(f"{name} must be a non-empty string")
    return value.strip()
...
def load_mediacrawler_lock(lock_path: Path) -> MediaCrawlerLock:
    ...
    try:
        if not resolved_lock.is_file() or resolved_lock.stat().st_size > MAX_LOCK_BYTES:
            raise CheckoutValidationError("upstream lock is missing or exceeds the size limit")
        payload = _mapping(json.loads(resolved_lock.read_text(encoding="utf-8")), "upstream lock")
    except (OSError, UnicodeError, json.JSONDecodeError) as error:
        raise CheckoutValidationError("upstream lock could not be read safely") from error

    if payload.get("schema_version") != 1:
        raise CheckoutValidationError("unsupported upstream lock schema")
    raw_upstreams = payload.get("upstreams")
    if not isinstance(raw_upstreams, list):
        raise CheckoutValidationError("upstream lock must contain an upstreams list")
    matches = [
    ...
    if len(matches) != 1:
        raise CheckoutValidationError("upstream lock must contain exactly one MediaCrawler entry")
    ...
    if repository != MEDIACRAWLER_REPOSITORY:
        raise CheckoutValidationError("MediaCrawler repository does not match the qualified source")
    if license_name != MEDIACRAWLER_LICENSE:
        raise CheckoutValidationError("MediaCrawler license does not match the qualified source")
    if _FULL_SHA.fullmatch(commit) is None:
        raise CheckoutValidationError("MediaCrawler commit must be a full lowercase Git SHA")
...
def _git_output(checkout: Path, *arguments: str) -> str:
    try:
        ...
        raise CheckoutValidationError("Git could not inspect the MediaCrawler checkout") from error
    if completed.returncode != 0:
        raise CheckoutValidationError("Git rejected the MediaCrawler checkout")
...
def _tracked_blob(checkout: Path, relative_path: str) -> str:
    ...
    if len(fields) != 4 or fields[0] not in {"100644", "100755"} or fields[3] != relative_path:
        raise CheckoutValidationError("MediaCrawler required entry is not a tracked regular file")
    ...
    if fields[1].lower() != committed_blob or working_blob != committed_blob:
        raise CheckoutValidationError("MediaCrawler required entry differs from the locked commit")
...
def verify_mediacrawler_python(python_executable: Path) -> VerifiedPython:
    ...
    if not executable.is_file() or not os.access(executable, os.X_OK):
        raise CheckoutValidationError("MediaCrawler Python executable is unavailable")
    try:
        completed = subprocess.run(
        ...
        raise CheckoutValidationError("MediaCrawler Python runtime probe could not complete") from error
    if completed.returncode == 41:
        raise CheckoutValidationError("MediaCrawler Python must be version 3.11 or newer")
    if completed.returncode != 0:
        raise CheckoutValidationError("MediaCrawler Python is missing required runtime imports")
...
def verify_mediacrawler_checkout(
    ...
    *,
    ...
    if not checkout.is_dir():
        raise CheckoutValidationError("MediaCrawler checkout is unavailable")
    repository_root = Path(_git_output(checkout, "rev-parse", "--show-toplevel")).resolve()
    if repository_root != checkout:
        raise CheckoutValidationError("MediaCrawler checkout path is not the Git repository root")
    if not (checkout / "main.py").is_file() or not (checkout / "config" / "__init__.py").is_file():
        raise CheckoutValidationError("MediaCrawler checkout is missing required entry points")
    ...
    try:
        ...
        raise CheckoutValidationError("MediaCrawler license file is unavailable") from error
    if license_header != lock.license_name:
        raise CheckoutValidationError("MediaCrawler checkout license does not match the lock")
    if hashlib.sha256(license_bytes).hexdigest() != MEDIACRAWLER_LICENSE_SHA256:
        raise CheckoutValidationError("MediaCrawler checkout license digest is not qualified")

    revision = _git_output(checkout, "rev-parse", "HEAD").lower()
    if revision != lock.commit:
        raise CheckoutValidationError("MediaCrawler checkout revision does not match the lock")
    ...
    if _git_output(
        ...
        raise CheckoutValidationError("MediaCrawler checkout contains modifications or untracked files")
