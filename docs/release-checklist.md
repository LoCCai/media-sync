...
- [ ] `[host]` The secure console is implemented and has a passing [local synthetic-browser verification](executions/0055-operator-auth-playback-evidence/secure-console/verification.md) result; root HTML or reachable health is not complete console/live usability. Record login→session, CSRF mutation, QR/direct-media/SSE and logout/expiry separately; P1 confirmation UI is not a CLI-canary prerequisite.
- [ ] `[host]` For the exact current image, override the entrypoint and run `serve --check-config` per [deployment](deployment.md), verify real runtime-UID credential reads and invalid configuration failing before migration. The command does not prove DNS/ports or full readiness. Keep unexecuted Linux/Docker gates NOT_RUN; do not reuse historical 0050 PASS.
