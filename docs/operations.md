...
The current entrypoint runs shared `serve --check-config` for `serve` (including `-- serve`) before Xvfb and idempotent `db init`; explicit check-only/help do not migrate. Migration can still occur after successful preflight, so preserve compatible upgrade backups and do not infer an untouched database from a later startup failure. Before upgrading, follow the [deployment guide](deployment.md) to run the same configuration preflight with the entrypoint overridden. It performs no DNS/bind work and does not prove current Linux mount permissions, port availability or readiness. Keep the credential restricted and readable by the effective mapped runtime user, not merely root-owned `0600`, world-readable or recursively re-owned. Docker/Linux UID gates have not run on this Windows workstation. Web login/session/memory-only CSRF is implemented with a passing [local synthetic-browser verification](executions/0055-operator-auth-playback-evidence/secure-console/verification.md) result; CLI/supervisor remains available. `uv.lock` pins dependencies, and the git-ignored live Compose file remains independently managed.
