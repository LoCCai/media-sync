...
- 执行 0053 活跃范围：保持既有内容/资产数组契约，同时增加有界服务端筛选与安全精确详情；通过只接受 UUID 的 GET/HEAD，以同描述符验证/流式读取和严格单 Range 语义服务经过验证的内容寻址归档字节；恢复复用持久 asset-download Operation；升级 Contents、Assets 与 Library 且不暴露 raw、locator 或宿主路径。媒体服务器树/控制留在 0054，鉴权/保留留在 0055。
