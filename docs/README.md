...
| 0008 | MediaCrawler acceptance closeout / MediaCrawler 验收收口 | Planned; implementation and behavioral verification `NOT_RUN` / 已计划；实现与行为验证均为 `NOT_RUN` | This bilingual planning commit / 本次双语计划提交 |
...
Execution 0008 is a planning baseline only. It freezes a narrow offline successor closeout for execution 0007 AC6/AC13: deterministic child-exit/pre-seal and post-seal/pre-ingest cancellation barriers, plus an eleven-row × three-sink secret matrix with an exact 33-cell completeness test. No implementation or behavioral gate has run. Signed-locator refresh is ordered after this closeout as execution 0009, and the durable automatic `sync → download → Emby` DAG remains execution 0010. See [`executions/0008-mediacrawler-acceptance-closeout/`](executions/0008-mediacrawler-acceptance-closeout/).

执行 0008 目前只是计划基线。它为执行 0007 的 AC6/AC13 冻结一个窄范围离线继任收口：确定性的 child-exit/pre-seal 与 post-seal/pre-ingest 取消 barrier，以及带精确 33-cell completeness 测试的“11 行 × 3 落点”密钥矩阵。尚未运行实现或行为门禁。签名 locator refresh 排在本次收口之后，作为执行 0009；持久自动 `sync → download → Emby` DAG 仍为执行 0010。详见 [`executions/0008-mediacrawler-acceptance-closeout/`](executions/0008-mediacrawler-acceptance-closeout/)。
