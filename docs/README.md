...
| 0009 | Provenance-bound signed-locator refresh / 按来源绑定的签名 locator 刷新 | Planned only; implementation and every live row `NOT_RUN` / 仅完成计划；实现及全部真人行保持 `NOT_RUN` | This bilingual planning commit / 本次双语计划提交 |
...
Execution 0009 currently freezes only the design and verification contract for exact Subscription/Account provenance, a private detail-only signed-URL channel, manual default-off refresh/download and successful/recovery terminal cleanup. No schema, helper, platform selector or download behavior from that plan has run yet. Automatic downstream Jobs remain execution 0010; all real platform/CDN/Emby rows remain `NOT_RUN`. See [`executions/0009-signed-locator-refresh/`](executions/0009-signed-locator-refresh/).

执行 0009 当前只冻结精确 Subscription/Account 来源、私有 detail-only 签名 URL 通道、默认关闭的手工刷新/下载，以及成功/恢复终态清理的设计与验证契约；该计划中的 schema、helper、平台 selector 与下载行为均尚未运行。自动下游 Job 继续属于执行 0010；全部真人平台/CDN/Emby 行保持 `NOT_RUN`。详见 [`executions/0009-signed-locator-refresh/`](executions/0009-signed-locator-refresh/)。
