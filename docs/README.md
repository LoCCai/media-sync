...
| 0009 | Provenance-bound signed-locator refresh / 按来源绑定的签名 locator 刷新 | Paused at a partial implementation checkpoint; gates failing; every live row `NOT_RUN` / 已在部分实现检查点暂停；门禁失败；全部真人行保持 `NOT_RUN` | Plan `6c43c83`; local WIP pause checkpoint / 计划 `6c43c83`；本地 WIP 暂停检查点 |
...
Execution 0009 is paused at a partial implementation checkpoint. The provenance migration/ORM/repository slice and part of successful/recovery terminal cleanup are present in the local WIP commit, but ingestion wiring, source/Job binding, private child protocol, platform selectors, downloader refresh, CLI wiring and acceptance gates remain incomplete. Mypy, focused migration tests and the handler regression file currently fail; manual refresh still returns `locator_refresh_unsupported`. Execution 0010 has not started and every real platform/CDN/Emby row remains `NOT_RUN`. See [`executions/0009-signed-locator-refresh/`](executions/0009-signed-locator-refresh/).

执行 0009 已在部分实现检查点暂停。本地 WIP 提交包含来源 migration/ORM/repository 切片及部分成功/恢复终态清理，但导入接线、来源/Job 绑定、私有 child 协议、平台 selector、下载器刷新、CLI 接线和验收门禁仍未完成；mypy、migration 专项及 handler 回归文件当前失败，手工刷新仍返回 `locator_refresh_unsupported`。执行 0010 尚未开始，全部真人平台/CDN/Emby 行保持 `NOT_RUN`。详见 [`executions/0009-signed-locator-refresh/`](executions/0009-signed-locator-refresh/)。
