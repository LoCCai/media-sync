...
| 0007 | MediaCrawler scheduled handler / MediaCrawler 定时 handler | Planned; implementation and verification `NOT_RUN` / 已计划；实现与验证均为 `NOT_RUN` | This bilingual planning commit / 本次双语计划提交 |
...
Execution 0007 is a planning baseline only. It proposes an opt-in, license-gated MediaCrawler scheduled handler with policy v1, manifest v3/receipt v2, attempt-scoped process roots, parent-owned lease heartbeat, cooperative and parent-death cancellation, exact ingestion fencing and safe failed-artifact cleanup. No execution 0007 implementation or automated/live verification has run; the currently shipped registry remains Fake-only. See [`executions/0007-mediacrawler-scheduled-handler/`](executions/0007-mediacrawler-scheduled-handler/) for the frozen goal, plan and `NOT_RUN` evidence matrix.

执行 0007 目前只是计划基线。它计划交付显式启用、受许可证约束的 MediaCrawler 定时 handler，包括 policy v1、manifest v3/receipt v2、attempt-scoped 进程根、父进程拥有的 lease heartbeat、协作取消与父死亡取消、精确导入 fencing，以及失败产物安全清理。执行 0007 尚未运行任何实现、自动验证或真人验证；当前随附 registry 仍仅支持 Fake。冻结目标、计划及 `NOT_RUN` 证据矩阵位于 [`executions/0007-mediacrawler-scheduled-handler/`](executions/0007-mediacrawler-scheduled-handler/)。
