...
| 0009 | Provenance-bound signed-locator refresh / 按来源绑定的签名 locator 刷新 | In progress; provenance/ingestion/cleanup focused gates pass; refresh/CLI pending; every live row `NOT_RUN` / 推进中；来源/导入/清理专项通过；refresh/CLI 待完成；全部真人行保持 `NOT_RUN` | Plan `6c43c83`; resumed local tranche / 计划 `6c43c83`；已恢复的本地批次 |
| 0010 | Automatic subscription media pipeline / 自动订阅媒体流水线 | In progress — functional MVP planning baseline; implementation `NOT_RUN` / 推进中——功能 MVP 计划基线；实现尚未运行 | [`executions/0010-automatic-media-pipeline/`](executions/0010-automatic-media-pipeline/) |
...
Execution 0009 has resumed with function-first priority. Migration/backfill, exact same-transaction provenance ingestion and successful/recovery terminal cleanup now pass their merged focused gates (`87 passed, 1 skipped`; strict mypy passes 65 source files). Source/Job selection, detail refresh, downloader/CLI wiring and the functional end-to-end path remain active work; manual refresh still returns `locator_refresh_unsupported` at this checkpoint. Execution 0010 has not started and every real platform/CDN/Emby row remains `NOT_RUN`. See [`executions/0009-signed-locator-refresh/`](executions/0009-signed-locator-refresh/).

执行 0009 已按功能优先恢复。Migration/backfill、精确同事务来源导入及成功/恢复终态清理已通过合并专项门禁（`87 passed, 1 skipped`；65 个源码文件严格 mypy 通过）。来源/Job 选择、detail refresh、下载器/CLI 接线及功能端到端路径仍在推进；本检查点的手工 refresh 仍返回 `locator_refresh_unsupported`。执行 0010 尚未开始，全部真人平台/CDN/Emby 行保持 `NOT_RUN`。详见 [`executions/0009-signed-locator-refresh/`](executions/0009-signed-locator-refresh/)。
