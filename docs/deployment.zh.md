...
抖音使用真实锁定JS签名；缺少localStorage.xmst时保留上游签名中的字面None并发送完全相同query，不猜造token或请求新token。这项离线一致性不保证真实平台接受。整数status_code0是本版保守响应子集。抖音/快手粘贴本人Cookie校验仍未开放，已有Cookie来源可用不等于新增了验证器。抖音/快手/贴吧目标资料查询不证明或更新本人认证。

贴吧头像只支持固定gss0.bdstatic.com来源路径且portrait与本次作者相同，可带同一响应中的时间戳；未知img_url不获取，缺失时使用源码证明的固定fallback。仍有2MiB/像素/超时/无重定向/无Cookie/私网拒绝和同源PNG规则。头像失败不阻塞昵称、不抹除旧头像，资料更新不移动归档或重命名导出。小红书资料、抖音/快手/知乎头像尚待接入。无需新增环境、依赖、schema或媒体服务器配置；本轮不部署生产、重试canary或恢复supervisor。

## 贴吧粘贴Cookie与快手/知乎昵称（0063）

另行更新/重建服务后，账户页可通过原有受保护弹窗校验保存贴吧请求Cookie头。只执行一次HTTPS本人请求，不扫码、不采集；严格正向远程身份证明后才保存。非零/未知码、坏身份及游客响应保留旧凭据。本版接受整数no=0、正uint64整数id及现代tb.1.portrait，旧/IP portrait或未证明的其他响应类型不宣称支持；本地BDUSS标记不能独立认证。抖音/快手粘贴本人校验仍未接入。

订阅页中，符合条件的已认证保存会话或既有Cookie账户可在输入完成（离开输入框/Enter）后单次查询快手/知乎昵称。快手输入精确opaque用户ID（最多128 ASCII字符），知乎输入url_token（最多255，排除单独点/双点），不是整段URL或数字ID转换；资料响应必须带相同身份。快手仅观察作者，不验证或改变本人登录；知乎先查本人端点再读一个精确作者页面。无投稿、媒体、扫码或自动重试请求；独立资料查询不需要采集的全历史确认。

这两个平台目前仅昵称，头像抓取尚未实现，不抓取未经证明的URL；B站/微博保留既有可选头像。失败/晚到刷新保留旧观察，昵称更新不重命名现有归档/导出路径。Cookie仍私密，不要发进issue。无需新增迁移、环境设置或媒体服务器连接；当前Linux与真实平台验收需另行进行，源码发布不等于授权部署、线上重试或恢复supervisor。
...
头像为可选同源PNG，隔离限量下载解码，HTTPS CDN规则绑定资料平台。窄新浪crop/尺寸路径及可选profile_image_url回退是合成设计合同，不是已经证明的真人URL形状；未知/带签名/重定向/不支持图片不获取，昵称和旧头像仍保留。0063/0064另接入快手/知乎/抖音/贴吧昵称及贴吧头像；小红书资料、抖音/快手/知乎头像仍待完成。0060无新依赖或数据库迁移。见[0060验证](executions/0060-weibo-creator-profile/verification.zh.md)，部署/真人验证须单独进行，不自动恢复supervisor。
...
账户页现提供 B 站、小红书、微博、知乎及0063新增贴吧的明确 **粘贴 → 远程本人认证校验 → 私密保存** 流程。只粘贴浏览器请求中的 Cookie 头值，不含 `Cookie:` 前缀、Set-Cookie 属性或 JSON 导出；不要把凭据发到聊天或附进支持报告。最多16 KiB、128个唯一键值对，保留值内等号和合法外层引号。抖音、快手仍缺少可靠远程身份验证器，明确标记不可用，不会凭本地 Cookie 标记判定已认证；贴吧的保守响应接受范围见本页0063说明。
