...
执行 0055 为该读取结果提供可安全用于确认的身份，并已由只允许浏览器的确认后端消费它。只有完整且唯一 `matched` 的 lookup，才会先在 profile、publication 与 selector fingerprint 上下文中哈希有界远端 item ID，再把该 item fingerprint 与规范作者绑定，派生 `observation_fingerprint`；`not_found` 会同时省略 item 与 observation fingerprint。`PlaybackEvidenceService.confirm` 只接受规范作者 UUID 与该不透明摘要，依次执行 resolve target A、一次完整唯一 lookup、resolve target B，比对两个 target 与重算身份，然后释放有界 authority lock，最后才打开短 create-or-replay 事务。该摘要标识操作者显式确认已播放的那次 observation；摘要与 service 都不会远程测量播放。当前尚无 Web 确认 UI 或安全 current/stale 读取投影消费持久证据。
...
- Revision `0008_playback_evidence` 提供确认 service 使用的持久账本。其仓储只暴露针对自然 observation 身份的 create-or-exact-replay，而且不提交调用方事务。首个持久行及其时间戳胜出；精确重放返回该行，用相同 observation fingerprint 搭配不同作者、publication Job 或上下文摘要则关闭失败。SQLite 在首次身份读取前以 `BEGIN IMMEDIATE` 预留写者，并拒绝已经开启的 deferred transaction；PostgreSQL 通过唯一约束及竞争插入后的 savepoint 恢复处理竞态。仅浏览器 POST 要求精确 Cookie/Origin/CSRF 权威，在进入 handler 前拒绝任意 Authorization header，严格只接受作者 UUID 与 fingerprint，且不返回 fingerprint 或内部上下文摘要。这些控制已经实现经鉴权写入权限，但在有界 current/stale 投影与 qualification schema v3 完成前，仍不构成整体资格能力，也不能授予真人 PASS。
