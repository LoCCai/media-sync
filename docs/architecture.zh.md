...
执行 0055 为该读取结果提供可安全用于确认的身份，并已由只允许浏览器的确认后端消费它。只有完整且唯一 `matched` 的 lookup，才会先在 profile、publication 与 selector fingerprint 上下文中哈希有界远端 item ID，再把该 item fingerprint 与规范作者绑定，派生 `observation_fingerprint`；`not_found` 会同时省略 item 与 observation fingerprint。`PlaybackEvidenceService.confirm` 只接受规范作者 UUID 与该不透明摘要，依次执行 resolve target A、一次完整唯一 lookup、resolve target B，比对两个 target 与重算身份，然后释放有界 authority lock，最后才打开短 create-or-replay 事务。该摘要标识操作者显式确认已播放的那次 observation；摘要与 service 都不会远程测量播放。有界作者查询现已消费持久证据：resolve A/profile A → 完整 lookup → resolve B/profile B 在短读取事务前建立当前权威。当前行按唯一 observation 身份独立查询；历史按确认时间／ID 排序，默认 20、最多 50 行，含 current 与分页哨兵最多物化 limit + 2 行。完整不存在使历史过期；远端失败、不完整遍历或身份变化使历史未知。安全响应不含摘要、selector、Job、原始路径或远端 ID。Web 确认 UI 仍待实现。
...
- Revision `0008_playback_evidence` 提供确认 service 使用的持久账本。其仓储写入边界只暴露针对自然 observation 身份的 create-or-exact-replay，而且不提交调用方事务。首个持久行及其时间戳胜出；精确重放返回该行，用相同 observation fingerprint 搭配不同作者、publication Job 或上下文摘要则关闭失败。SQLite 在首次身份读取前以 `BEGIN IMMEDIATE` 预留写者，并拒绝已经开启的 deferred transaction；PostgreSQL 通过唯一约束及竞争插入后的 savepoint 恢复处理竞态。仅浏览器 POST 要求精确 Cookie/Origin/CSRF 权威，在进入 handler 前拒绝任意 Authorization header，严格只接受作者 UUID 与 fingerprint，且不返回 fingerprint 或内部上下文摘要。经鉴权作者 GET 返回安全 current/history 投影；资格 v3 只有在精确持久确认匹配重新验证的当前权威时，才对显式选定作者授予 PASS。历史分页不会隐藏独立查询的当前行。
