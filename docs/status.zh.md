...
| Docker 打包 | 候选镜像、可复现加固及最终镜像不含 Node 的 Console v2 多阶段构建已交付（0041、0048–0050）；操作者首个镜像已构建/启动并暴露 venv launcher 缺陷，现已修复，待无缓存重建 |
...
| Docker 镜像构建 | `PARTIAL`：操作者的 `4c6d0bf` 镜像构建/启动成功，清单与运行用户直接启动 Chromium `151.0.7922.34` 通过，但 doctor 失败于 `runtime_invalid / runtime_imports_missing`。venv 符号链接修复与构建期非特权 doctor 已纳入下一次无缓存重建 | 执行 0050 验证；阶段 B 仍是第一发布阻塞项 |
...
1. Linux 主机基线未完成（修复版镜像无缓存构建、doctor/深度预检全绿、健康、重启持久性、备份恢复演练）——阶段 B。
