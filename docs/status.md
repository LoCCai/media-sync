...
| Docker packaging | Candidate image plus reproducibility hardening and the Node-free multi-stage Console v2 build delivered (0041, 0048–0050); the first operator image built/started and exposed a venv-launcher defect, now repaired pending a no-cache rebuild |
...
| Docker image build | `PARTIAL`: the operator's `4c6d0bf` image built/started; manifest and direct runtime-user Chromium `151.0.7922.34` passed, while doctor failed `runtime_invalid / runtime_imports_missing`. The venv-symlink repair and build-time unprivileged doctor are committed for the next no-cache rebuild | Execution 0050 verification; Phase B remains the first release blocker |
...
1. Linux-host baseline incomplete (repaired no-cache image build, green doctor/deep preflight, health, restart persistence, backup-restore drill) — Phase B.
