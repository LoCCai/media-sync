...
| Offline complete suite | Latest: execution 0049 authoring-station `2066 passed, 1 skipped` (full green run). History: execution 0048 recorded nondeterministic 33/35-failure child-process sealing runs on the same station, so Windows-native runs stay Experimental pending classification; the Linux phase-B rerun remains the authority | Executions 0048 and 0049 verification; the junit artifact is a workstation-local file under `artifacts/` (raw XML is git-ignored — the repository keeps only the sanitized summary and the artifact convention) |
