...
## Latest: deployment login repair (2026-09-05)

The operator successfully entered the deployed console manually. Existing Bilibili, Douyin and Xiaohongshu accounts showed failed login sessions; six historical login operations failed. These are observed failures, not successful qualification. The shared browser-cache environment, five upstream Chrome-channel selectors and headed-preflight mismatch are now repaired locally under frozen plan `204655d`; the container also waits for Xvfb connectivity before migrating. A real Windows blank headed Chromium launch passed (`151.0.7922.34`). Final regression/publication evidence and remaining work are tracked in [login-runtime verification](executions/0055-operator-auth-playback-evidence/login-runtime/verification.md). The repaired Linux image, QR scan, saved-session reuse, capture and Emby/Jellyfin canaries remain unverified; the overall seven-platform goal stays open. Older checkpoint results below remain historical unless explicitly refreshed by that record.
