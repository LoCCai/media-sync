...
范围：执行 0046 首次建立的逐项自审，并校准到执行 0054 阶段 B 的已实现姿态；每项声明均附强制机制。这是自审，不是外部审计。
...
| 无鉴权是已记录的决策 | API 是本地优先的操作者界面，容器部署文档要求可信网络。所有媒体服务器网络动作（包括 probe、两种 scan 模式与作者 lookup）另受默认关闭的 `MEDIA_SYNC_MEDIA_SERVER_OPERATIONS_ENABLED` 服务端门约束，但该门不是鉴权 |
...
1. API 无鉴权：能访问宿主机端口的人即可控制服务。执行 0054 默认关闭媒体服务器操作，但操作者一旦打开该门，任何能访问 API 的客户端都能提交作者查找、连接探测、legacy 只确认接受的刷新或作者刷新并核验。缓解：保持回环/Tailscale/可信局域网、闲置时关闭该门，并把执行 0055 的操作者鉴权视为尚未解决的风险，不能宣称已有访问控制。
