...
The offline-qualified refresh shapes are XHS image/video, Douyin image/video/audio/cover, Kuaishou video/cover, Bilibili cover, and Bilibili ordinary-upload logical-first-page video only when the play response has exactly one progressive `durl`. XHS still needs an operator-supplied exact-note detail reference and does not discover authority for multiple notes automatically. Weibo, Tieba and Zhihu have no normalized downloadable Asset. Bilibili DASH/mux, multi-segment, multi-page, subtitle and danmaku remain unsupported. Query-free `direct` locators continue to use the platform-independent downloader without enabling MediaCrawler.

通过离线验收的刷新形状为小红书 image/video、抖音 image/video/audio/cover、快手 video/cover、Bilibili cover，以及“播放响应精确包含一个 progressive `durl`”时的 Bilibili 普通投稿逻辑首 P 视频。小红书仍需操作员提供精确 note 详情引用，不能自动发现多个 note 的权限。微博、贴吧、知乎没有已归一化的可下载 Asset；Bilibili DASH/mux、多段、多 P、字幕与弹幕仍不支持。无 query 的 `direct` locator 继续使用平台无关下载器，无需启用 MediaCrawler。
...
Execution 0009's refresh MVP, execution 0010's explicit bounded downstream pipeline, execution 0012's local foreground supervisor and execution 0013's Bilibili first-page single-progressive video are implemented and pass their focused offline gates. Execution 0013 passes 223 focused tests; the complete suite passes 1199 with one Windows-inapplicable skip. This evidence proves repository selection, private detail refresh, deterministic/mock download, durable child recovery, local Emby layout publication and the frozen Bilibili slice; it does not prove any live platform/CDN/media-server row. The production pipeline handler is synchronous and runs through `asyncio.to_thread`; the supervisor therefore drains one already-active attempt under heartbeat instead of cancelling it. Forced underlying-thread termination and multi-worker HA remain unqualified.

执行 0009 刷新 MVP、执行 0010 显式有界下游 pipeline、执行 0012 本地前台监督器及执行 0013 Bilibili 首 P 单 progressive 视频均已实现并通过各自离线专项门禁。执行 0013 专项通过 223 项；完整套件通过 1199 项，另有一项 Windows 不适用而跳过。这些证据证明 repository 选择、私有 detail 刷新、确定性/mock 下载、持久 child 恢复、本地 Emby 布局发布及冻结的 Bilibili 切片；不证明任何真人平台/CDN/媒体服务器行。生产 pipeline handler 为同步函数并通过 `asyncio.to_thread` 运行，因此监督器会在 heartbeat 下等待一项已经 active 的尝试，而不是取消它。底层线程强制终止与多 worker HA 尚未验收。
