...
Execution 0009 has resumed. The many-to-many Asset/Subscription migration/backfill, repository eligibility, same-transaction ingestion observation and fresh/recovered/restart terminal cleanup now pass focused merged gates. Detail refresh, source/Job binding and downloader/CLI wiring remain unavailable at this checkpoint, so manual adapter-refresh download still returns `locator_refresh_unsupported`.

执行 0009 已恢复推进。多对多 Asset/Subscription migration/backfill、repository eligibility、同事务导入 observation 及 fresh/recovered/restart 终态清理现已通过合并专项门禁。本检查点的 detail refresh、来源/Job 绑定与下载器/CLI 接线仍不可用，因此手工 adapter-refresh 下载仍返回 `locator_refresh_unsupported`。
...
Execution 0008 now closes only those two gaps as successor offline evidence. The deterministic child-exit/pre-seal and single/repeated post-seal/pre-ingest barriers pass, and the exact eleven-failure × three-sink matrix proves 33 cells with fail-closed retained-filesystem/SQLite scans and fixed operator authority. The full suite passes 837 tests with one Windows-inapplicable skip and 79% branch-aware coverage. Execution 0009 is `PARTIAL / PASSING FOCUSED GATES` after resume; public refresh capability is still pending. Durable automatic downstream planning remains execution 0010 and has not started.

执行 0008 现只以继任离线证据关闭上述两个缺口。确定性 child-exit/pre-seal 及单次/重复 post-seal/pre-ingest barrier 通过；精确“11 种失败 × 3 类落点”矩阵以 fail-closed 留存文件系统/SQLite 扫描及固定运维权限证明 33 个 cell。完整套件通过 837 项测试、1 项 Windows 不适用的 skip，分支感知覆盖率 79%。执行 0009 恢复后为 `PARTIAL / PASSING FOCUSED GATES`，公开 refresh 能力仍待完成；持久自动下游规划仍属于尚未开始的执行 0010。
