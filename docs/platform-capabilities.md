...
The offline-qualified refresh shapes are XHS image/video, Douyin image/video/audio/cover, Kuaishou ordinary single video plus optional cover under execution 0014's exact-one-play-URL boundary, Bilibili cover, and Bilibili ordinary-upload logical-first-page video only when the play response has exactly one progressive `durl`. Composed playable qualification is narrower: Bilibili logical first-page single progressive, Kuaishou ordinary single video plus optional cover, and execution 0015's Douyin ordinary numeric-ID single video plus optional cover with empty note/music fields. XHS still needs an operator-supplied exact-note detail reference and does not discover authority for multiple notes automatically. Weibo, Tieba and Zhihu have no normalized downloadable Asset. Douyin gallery/associated-music/multiple-URL composition, Kuaishou galleries/multiple URLs and Bilibili DASH/mux, multi-segment, multi-page, subtitle and danmaku remain unsupported or unqualified. Query-free `direct` locators continue to use the platform-independent downloader without enabling MediaCrawler.

通过离线验收的刷新形状为小红书 image/video、抖音 image/video/audio/cover、执行 0014“精确一个播放 URL”边界下的快手普通单视频与可选封面、Bilibili cover，以及“播放响应精确包含一个 progressive `durl`”时的 Bilibili 普通投稿逻辑首 P 视频。组合可播放验收范围更窄：Bilibili 逻辑首 P 单 progressive、快手普通单视频与可选封面，以及执行 0015 中 note/music 为空的抖音普通 numeric-ID 单视频与可选封面。小红书仍需操作员提供精确 note 详情引用，不能自动发现多个 note 的权限。微博、贴吧、知乎没有已归一化的可下载 Asset；抖音图集/关联音乐/多 URL 组合、快手图集/多 URL，以及 Bilibili DASH/mux、多段、多 P、字幕与弹幕仍不支持或未验收。无 query 的 `direct` locator 继续使用平台无关下载器，无需启用 MediaCrawler。
...
Execution 0009's refresh MVP, execution 0010's explicit bounded downstream pipeline, execution 0012's local foreground supervisor, execution 0013's Bilibili first-page single-progressive video, execution 0014's Kuaishou ordinary single-video+cover shape and execution 0015's Douyin ordinary single-video+cover shape are implemented and pass their focused offline gates. Execution 0015 passes 231 focused tests; the complete suite passes 1209 with one Windows-inapplicable skip. This evidence proves repository selection, private detail refresh, deterministic/mock download, durable child recovery, local Emby layout publication and the three frozen playable-platform slices; it does not prove any live platform/CDN/media-server row. The production pipeline handler is synchronous and runs through `asyncio.to_thread`; the supervisor therefore drains one already-active attempt under heartbeat instead of cancelling it. Forced underlying-thread termination and multi-worker HA remain unqualified.

执行 0009 刷新 MVP、执行 0010 显式有界下游 pipeline、执行 0012 本地前台监督器、执行 0013 Bilibili 首 P 单 progressive 视频、执行 0014 快手普通单视频+封面形状及执行 0015 抖音普通单视频+封面形状均已实现并通过各自离线专项门禁。执行 0015 专项通过 231 项；完整套件通过 1209 项，另有一项 Windows 不适用而跳过。这些证据证明 repository 选择、私有 detail 刷新、确定性/mock 下载、持久 child 恢复、本地 Emby 布局发布及三个冻结的可播放平台切片；不证明任何真人平台/CDN/媒体服务器行。生产 pipeline handler 为同步函数并通过 `asyncio.to_thread` 运行，因此监督器会在 heartbeat 下等待一项已经 active 的尝试，而不是取消它。底层线程强制终止与多 worker HA 尚未验收。

Platform-specific DASH/multi-part/subtitle/danmaku and slideshow/mux derivatives, XHS multi-note authority discovery, per-request HTTP spacing, forced synchronous-thread cancellation/multi-worker HA, REST operations, Docker and production service operations remain unavailable or deferred implementation scope, not `NOT_RUN` qualification outcomes. The shipped supervisor is an explicit single-host foreground process, not a production service. The only upstream pacing evidence is configuration of `CRAWLER_MAX_SLEEP_SEC` together with `MAX_CONCURRENCY_NUM=1`; it is not a guarantee for every HTTP request.

平台特有 DASH/多 P/字幕/弹幕及幻灯片/mux 衍生物、小红书多 note 权限发现、逐 HTTP 请求间隔、同步线程强制取消/多 worker HA、REST 运维、Docker 与生产服务运维继续属于不可用或延期实现范围，不是 `NOT_RUN` 验收结果。已交付的监督器是显式单主机前台进程，不是生产服务。上游节奏方面唯一已有证据是同时配置 `CRAWLER_MAX_SLEEP_SEC` 与 `MAX_CONCURRENCY_NUM=1`；这不是每次 HTTP 请求的间隔保证。
