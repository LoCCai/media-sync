...
Execution 0009 is paused at a partial implementation checkpoint. The many-to-many Asset/Subscription migration/ORM/repository slice and part of fresh/recovered/restart terminal cleanup have landed locally, including generation advancement for semantic or persisted-locator replacement and preservation of post-commit database truth. They are not accepted: migration and handler regression gates fail, ingestion observation is not wired, and the private child, Job source binding, platform selectors, downloader refresh and CLI are absent. Manual download therefore still returns `locator_refresh_unsupported`.

执行 0009 已在部分实现检查点暂停。多对多 Asset/Subscription migration/ORM/repository 切片及部分 fresh/recovered/restart 终态清理已在本地落盘，包括 semantic 或持久 locator 替换推进 generation，以及保护成功提交后的数据库事实。但这些尚未验收：migration 与 handler 回归门禁失败，导入 observation 未接线，私有 child、Job 来源绑定、平台 selector、下载器刷新和 CLI 均缺失；因此手工下载仍返回 `locator_refresh_unsupported`。
...
Execution 0008 now closes only those two gaps as successor offline evidence. The deterministic child-exit/pre-seal and single/repeated post-seal/pre-ingest barriers pass, and the exact eleven-failure × three-sink matrix proves 33 cells with fail-closed retained-filesystem/SQLite scans and fixed operator authority. The full suite passes 837 tests with one Windows-inapplicable skip and 79% branch-aware coverage. Execution 0009 is `PARTIAL / FAILING` at its pause checkpoint and changes no public capability. Durable automatic downstream planning remains execution 0010 and has not started.

执行 0008 现只以继任离线证据关闭上述两个缺口。确定性 child-exit/pre-seal 及单次/重复 post-seal/pre-ingest barrier 通过；精确“11 种失败 × 3 类落点”矩阵以 fail-closed 留存文件系统/SQLite 扫描及固定运维权限证明 33 个 cell。完整套件通过 837 项测试、1 项 Windows 不适用的 skip，分支感知覆盖率 79%。执行 0009 在暂停检查点为 `PARTIAL / FAILING`，不改变公开能力；持久自动下游规划仍属于尚未开始的执行 0010。
