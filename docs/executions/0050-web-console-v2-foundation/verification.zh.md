...
| 修复版 Docker 构建/运行 | 操作者 Linux 主机，`0d73ba1` 之后 | 0050 切片 `PASS`——构建/启动、应用 doctor、深度预检与运行时 Chromium 启动均通过 |
...
- Dockerfile 现在会在构建期以非特权 `mediasync` 用户执行同一个 doctor。重建后 doctor 与深度预检均返回 `ready`，运行时 Chromium `151.0.7922.34` 与构建清单一致。
...
暂不宣称重启/备份演练、Linux 完整套件、宿主机端口验证、真人账户登录、采集、CDN 下载或 Emby/Jellyfin 扫描/播放。剩余阶段 B 与真人行继续归执行 0047。
