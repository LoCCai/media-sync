...
| 检查 | 命令 | 退出码 | 结果 |
| --- | --- | ---: | --- |
| 打包文件齐备且一致 | `Dockerfile`、`docker/entrypoint.sh`、`docker-compose.example.yml`、`.dockerignore`、`docs/deployment.md`、`docs/deployment.zh.md` | 0 | 入口脚本可执行位已提交；compose 只引用存在的命令（`serve`、`scheduler supervise --idle-interval-seconds`） |
...
| 验收行 | 命令 | 结果 |
| --- | --- | --- |
| 镜像构建 | `cp docker-compose.example.yml docker-compose.yml && docker compose build` | 本工作站 `NOT_RUN` |
