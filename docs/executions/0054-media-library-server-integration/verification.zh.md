...
本基线不宣称任何真实媒体服务器或平台结果。工作区没有配置 Emby/Jellyfin URL、key 或 library ID。Mock transport 和临时导出树只能证明封闭协议与文件系统契约。真实连接/版本/library 发现与定向刷新接受，在有操作者证据前保持 `NOT_RUN`；扫描完成轮询、项目查找、播放证据及自动联动在阶段 A 为 `NOT_IMPLEMENTED`。Linux 主机行为、平台账户/API 与 CDN 字节继续保持 `NOT_RUN`。
