...
No live media-server or platform claim is made by this baseline. The workspace has no configured Emby/Jellyfin URL, key, or library ID. Mock transports and temporary exported trees can prove closed protocol and filesystem contracts only. Real connection/version/library discovery, scan completion, item lookup, sample playback, automatic scanning, Linux host behavior, platform accounts/APIs, and CDN bytes stay `NOT_RUN` until operator evidence exists.
