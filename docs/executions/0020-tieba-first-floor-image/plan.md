...
1. **Freeze source and response contracts / 冻结源码与响应合约 — in progress / 推进中**
...
2. **Capture exact first-floor media and bound creators / 捕获精确首楼媒体并约束作者 — pending / 待执行**
...
3. **Normalize and refresh one ARTICLE-owned IMAGE / 归一化并刷新 ARTICLE 所属单图 — pending / 待执行**
...
4. **Qualify bytes and compose Emby output / 校验字节并组合 Emby 输出 — pending / 待执行**
...
5. **Verify, review and publish / 验证、审查与发布 — pending / 待执行**
...
1. `PENDING` — `docs: 启动贴吧首楼图片闭环 / start Tieba first-floor image pipeline`
2. `PENDING` — `feat: 闭环贴吧首楼图片 / close Tieba first-floor image pipeline`
3. `PENDING` — `docs: 收尾贴吧首楼图片闭环 / close Tieba first-floor image pipeline`
