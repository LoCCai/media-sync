...
The combined repair was published as `1e487f4`, with identical local/remote hashes and a clean worktree after fetch. Refresh the accounts page after updating the image, preflight one account, then let the operator start and scan its QR. Record the actual outcome before widening platforms. Exact deployed SHA/image digest, new-image build/QR scan/session reuse, capture and Emby/Jellyfin are still unverified. Pasted-Cookie login has an accepted goal and [draft plan](../cookie-login/plan.md), but no implementation in this increment.
