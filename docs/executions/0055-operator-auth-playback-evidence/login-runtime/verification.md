...
## Subsequent operator-supplied Linux smoke (2026-09-05)

After the handoff, the operator supplied successful output from the configuration-only check, service recreation and in-container blank-browser probe: `configuration: valid`, then `ok: true`, `browser: bundled-chromium`, `mode: headed-persistent`, version `151.0.7922.34`, and `live_qualification: NOT_RUN`. This is operator-supplied evidence of a real Linux container blank-browser launch, not another local Windows test. The previous statement that this particular server smoke had not run is superseded by this result.

The pasted output does not identify the exact Git SHA/image digest, and was not independently re-executed by the agent. Complete current-image qualification, startup/restart/restore, QR delivery/scan, saved-session reuse, capture and Emby/Jellyfin remain open. The operator was asked to refresh accounts and manually retry one Bilibili account before the remaining platforms. No platform login was started automatically and no platform PASS is recorded.
