...
- The 2026-09-12 [deployment record](deployment-2026-09-12.md) closes that gap on the Linux server: backup, clean fast-forward to `3c31dce`, upstream verification, one-arg Compose merge, no-cache rebuild, matching OCI label/`BUILD-MANIFEST`, healthy recreation, current `0015_xhs_creator_notes` and identical API/supervisor deep reports. Deployment identity stages are complete; the untagged pre-0077 image is retained for rollback.
- Live XHS login/profile adoption, creator API, CDN download, host archive/library/NFO bytes, replay, restart continuation and one resident-supervisor cycle remain `NOT_RUN` and are operator-gated by the [deployment handoff](deployment-handoff.md).
