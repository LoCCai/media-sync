...
- 2026-09-12 的[部署记录](deployment-2026-09-12.zh.md)在 Linux 服务器上补齐该缺口：备份、干净 fast-forward 到 `3c31dce`、上游校验、单参数 Compose 合并、无缓存重建、OCI 标签/`BUILD-MANIFEST` 一致、健康重建实例、当前 `0015_xhs_creator_notes` 及完全一致的 API/supervisor 深度报告。部署身份阶段完成；0077 前旧镜像保留为未标记回滚镜像。
- 真人 XHS 登录/profile 认领、作者 API、CDN 下载、宿主 archive/library/NFO 字节、重放、重启续跑及一次驻留 supervisor 周期仍为 `NOT_RUN`，由[部署交接](deployment-handoff.zh.md)设置操作员门槛。
