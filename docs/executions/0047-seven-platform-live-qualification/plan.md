...
1. `git pull && uv sync --all-groups --locked && uv run pytest -q` — record exact numbers and investigate every platform-specific divergence. The current 0055 browser-only confirmation checkpoint on Windows passes `2941 passed, 22 skipped, 1 warning in 594.72s`; 19 skips are real-PostgreSQL races without a configured URL (11 Operation plus 8 PlaybackEvidence). This result does not replace the complete Linux host gate with PostgreSQL enabled.
...
**Qualification hold point:** the backend authentication boundary, revision `0008_playback_evidence`, append-only persistence, and TOCTOU-safe browser-only confirmation service/API are implemented and offline-verified. Console v2 and `/legacy` still lack operator login/session/CSRF integration and confirmation UI, while the safe bounded by-author current/stale projection and qualification schema v3 remain unimplemented. The current schema-v2 qualification API therefore still reports overall `playback_evidence` as `NOT_IMPLEMENTED` with null human status. Do not start the Web login, live-platform, real media-server playback or phases C–F qualification steps from this checkpoint; keep every live qualification row `NOT_RUN`. Resume only after the projection/v3 and Web checkpoints have verified closeouts. Backend implementation or local/mock evidence alone grants no live PASS.
