...
Execution 0073 closes the smallest remaining seam between the authenticated native MediaCrawler console delivered in execution 0072 and media-sync's existing account-scoped subscription pipeline. The locked upstream `/crawler/` console becomes the primary place for QR and Cookie login. After the operator completes login there, they explicitly adopt a quiescent snapshot of that platform's saved browser profile into one existing media-sync Account. A successful, non-interactive validation publishes the Account as `saved_session` / `authenticated`; the existing scheduler, JSONL ingestion, controlled download, archive, library-folder and NFO path then continues unchanged. The WebUI source remains available for later native login renewal, while the scheduler uses only the independently installed Account snapshot.

This execution does not build another login implementation, crawler, downloader or publication pipeline. It also does not claim that all seven platforms have passed human login or live crawling. The adoption boundary copies and independently verifies browser state; it does not infer authentication from the presence of files, a WebUI success label or a previous platform result.
...
5. media-sync takes an exclusive quiescent snapshot of the WebUI profile, validates the snapshot through the existing saved-session boundary with QR fallback disabled, then atomically installs it into the selected Account's managed profile location.
...
3. **Runtime profiles are never shared** — the WebUI keeps its operator-controlled source profile, while each Account receives an independent, validated snapshot. Every initial adoption or later refresh is an explicit action fenced by the Account authentication revision; the scheduler never reads the live WebUI directory and no Account shares another Account's profile.
4. **Only quiescent state crosses the boundary** — reject adoption while the upstream crawler/login process or the selected Account's scheduler/profile lock is active. Hold the applicable WebUI runtime gate while snapshotting and the Account lock while replacing/publishing so Chromium state is never copied or replaced concurrently.
5. **Filesystem handling is confined and reversible** — accept only the expected non-empty directory beneath `webui-profiles/<platform>_user_data_dir`; reject traversal and prevent symlink/reparse escape or unsupported entries from crossing the boundary. Stage and replace only beneath the configured MediaCrawler runtime root. A failed copy, validation or publication keeps the WebUI source intact, restores any prior Account profile where safely possible, leaves the Account authentication row unchanged and emits a fixed safe failure classification.
...
7. **Account publication is atomic and fenced** — after validation, re-check Account identity, platform, auth revision and lock ownership before committing `saved_session` / `authenticated`. Clear incompatible credential references according to the existing invariant, retain no client-supplied profile path, and return only bounded adoption evidence without Cookie values, local profile contents or sensitive upstream output.
8. **Handled failure does not corrupt either side** — pre-start cancellation, probe timeout/failure, stale Account revision and publication conflict leave a truthful retryable outcome. A failed validation must not authenticate the Account; a late result must not cross a lost fence; the source remains usable and no partial destination may be selected by the scheduler. Abrupt service/process termination during copy or replacement remains an explicit unverified recovery risk for this slice.
...
11. **Qualification remains honest** — deterministic tests cover route/auth/CSRF checks, missing/empty profiles, WebUI and Account busy state, failed probes, stale revisions, successful validation/publication, prior-profile restoration and the adopted-profile scheduler path. These tests do not qualify real QR/Cookie login, platform requests, CDN bytes or seven-platform behavior. Live rows remain `NOT_RUN` until individually observed and recorded.
