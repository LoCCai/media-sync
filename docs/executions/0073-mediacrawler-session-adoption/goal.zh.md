...
执行 0073 关闭执行 0072 已交付的认证原生 MediaCrawler 控制台与 media-sync 既有账户级订阅流水线之间最小的一条接缝。锁定上游 `/crawler/` 控制台成为二维码和 Cookie 登录的主要入口。操作者在其中完成登录后，显式把该平台保存的浏览器 profile 认领给一个已有 media-sync Account。只有非交互式验证成功，才把 Account 发布为 `saved_session` / `authenticated`；随后继续使用既有 scheduler、JSONL 摄取、受控下载、归档、媒体库目录与 NFO 主链，不重写这些能力。

本执行不再实现另一套登录、爬虫、下载器或发布流水线，也不声称七个平台已经通过真人登录或真实采集。认领边界只转交经过验证的浏览器状态；文件存在、WebUI 成功字样或另一个平台曾经成功都不能推导出认证成功。
...
5. media-sync 在独占条件下把已静止的 WebUI profile 转移到所选 Account 的受管 profile 位置，并通过现有 saved-session 边界验证，禁止回退二维码。
...
3. **一个 profile 只有一个所有者** —— 认领是移动／取得所有权，不是共享复制。同一 WebUI 平台 profile 只能认领给一个 Account，且不得静默覆盖 Account 中非空的受管 profile。重新认领或替换必须另行设计显式且冲突安全的流程，不能隐式覆盖。
4. **只有静止状态可以跨边界** —— 上游 crawler／登录进程仍在运行，或所选 Account 的 scheduler／profile 锁被占用时，拒绝认领。在转移、验证、发布或回滚完成前持有相应 WebUI runtime 锁和 Account 锁，禁止并发读取或移动 Chromium profile。
5. **文件系统处理受限且可恢复** —— 只接受位于 `webui-profiles/<platform>_user_data_dir` 的预期非空目录；拒绝路径穿越、符号链接／reparse 逃逸及不支持的文件类型。暂存和移动只能发生在配置的 MediaCrawler runtime 根下。转移或验证失败时，在安全可行的情况下恢复来源 profile，保持 Account 不变，并产生固定、安全的失败分类。
...
7. **Account 发布原子且受 fencing 保护** —— 验证后必须重新检查 Account 身份、平台、认证 revision 及锁所有权，才可提交 `saved_session` / `authenticated`。按照既有不变量清除不兼容凭据引用，不保留客户端提供的 profile 路径，并记录有界 Operation/Event 结果；不得记录 Cookie 值、本地 profile 内容或敏感上游输出。
8. **失败不得破坏任一侧** —— 取消、超时、进程失败、Account revision 过期、发布冲突及服务关闭都必须留下可重试且准确的状态。验证失败不得认证 Account，丢失 fence 后的迟到成功不得发布，部分目标目录不得被 scheduler 选用。
...
11. **资格表述必须准确** —— 确定性测试必须覆盖路由／认证／CSRF、平台不匹配、空／不安全 profile、独占性、锁占用、成功验证／发布及每个回滚边界。这些测试不赋予真人二维码／Cookie 登录、平台请求、CDN 字节或七平台行为资格；各真人行在逐项实际观察并记录前继续为 `NOT_RUN`。
