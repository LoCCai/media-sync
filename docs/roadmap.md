...
- Execution 0013 planned slice: add one Bilibili ordinary-upload, first-page, single-segment progressive video Asset and compose its ephemeral play-url refresh through the existing downloader/probe/archive/Emby pipeline. DASH, split audio/video, multi-segment, multi-page, subtitle and danmaku remain explicitly unsupported; real account/CDN/media-server rows remain `NOT_RUN`. / 执行 0013 计划切片：为 Bilibili 普通投稿增加一个首 P、单段 progressive 视频 Asset，并把其瞬态播放地址刷新接入既有下载/探测/归档/Emby pipeline。DASH、音视频分离、多段、多 P、字幕与弹幕继续明确为不支持；真人账户/CDN/媒体服务器行保持 `NOT_RUN`。
