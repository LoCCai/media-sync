...
- Execution 0015 planned slice: qualify one ordinary numeric-ID Douyin record with an empty image list, exactly one video URL, no associated-music Asset and an optional cover through pure-ID detail, exact Account/Subscription refresh, default-profile deterministic download, controlled probing, archive and Emby `.mp4`/poster publication. It must first close unknown-query/userinfo/fragment/nested-shape retention across all Douyin media raw fields, including comma-list handling, without claiming bounded creator pagination, galleries/audio semantics, special CDN headers or any live result. / 执行 0015 计划切片：验收一条普通 numeric-ID 抖音记录，其图片列表为空、精确一个视频 URL、无关联音乐 Asset、封面可选；路径经过纯 ID detail、精确 Account/Subscription 刷新、默认 profile 确定性下载、受控探测、归档及 Emby `.mp4`/海报发布。它必须先关闭全部抖音媒体 raw 字段中未知 query/userinfo/fragment/嵌套形状留存（包括逗号列表处理），且不宣称作者分页有界、图集/音频语义、专用 CDN header 或任何真人结果。
