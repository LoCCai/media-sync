...
- Status: execution 0006 delivers the offline-qualified single-host scheduler foundation, execution 0007 adds the default-off pinned MediaCrawler handler, and execution 0008 closes its two remaining successor offline evidence gaps. Execution 0009 now has a frozen plan for exact refresh provenance, a dedicated private detail result pipe, explicit manual signed-locator download and successful/recovery artifact terminal cleanup, but none of that runtime behavior has been implemented or qualified yet. Per-request HTTP spacing, automatic downstream planning, REST, resident supervision and production packaging remain later executions; every live qualification row remains `NOT_RUN`.
- 状态：执行 0006 已交付通过离线验收的单机调度基础，执行 0007 新增默认关闭的锁定版 MediaCrawler handler，执行 0008 收口其余两项继任离线证据缺口。执行 0009 现已冻结精确刷新来源、专用私有 detail 结果 pipe、显式手工签名 locator 下载及成功/恢复产物终态清理计划，但这些运行行为均尚未实现或验收。逐 HTTP 请求间隔、自动下游规划、REST、常驻守护及生产打包仍属于后续执行；全部真人资格行继续为 `NOT_RUN`。
...
- Frozen order after execution 0008: execution 0009 first implements and offline-qualifies exact provenance-bound manual signed-locator refresh plus successful/recovery terminal cleanup; execution 0010 then adds the durable automatic `sync → download → Emby` DAG. The 0009 planning commit itself changes no capability. REST/operations and production supervision remain later work; authorized live platform/CDN and Emby qualification remains Phase 5 evidence. / 执行 0008 后的冻结顺序：执行 0009 先实现并离线验收按精确来源绑定的手工签名 locator refresh 及成功/恢复终态清理；执行 0010 再新增持久自动 `sync → download → Emby` DAG。0009 计划提交本身不改变能力。REST/运维与生产监督仍属于后续工作；经授权的真人平台/CDN 与 Emby 验收仍属于 Phase 5 证据。
