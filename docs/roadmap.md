...
- Status: the execution 0005 platform-neutral offline foundation is complete. Execution 0013 adds one Bilibili logical-first-page single-progressive shape, execution 0014 qualifies one Kuaishou ordinary single-video plus optional-cover shape, and execution 0015 qualifies one Douyin ordinary numeric-ID single-video plus optional-cover shape with empty note/music fields. All three compose exact refresh, controlled probe/archive and Emby `.mp4` publication; the latter two also publish posters. The downloader retains lock/scope, archive-to-database recovery and closed secret sinks under the documented trusted-runtime-root boundary. Real signed CDN retrieval and Emby/Jellyfin rescans/playback are outside automated acceptance and remain `NOT_RUN`.
- 状态：执行 0005 的平台无关离线基础已完成。执行 0013 新增一个 Bilibili 逻辑首 P 单 progressive 形状；执行 0014 验收一个快手普通单视频与可选封面；执行 0015 验收一个 note/music 为空的抖音普通 numeric-ID 单视频与可选封面。三个形状都组合了精确刷新、受控探测/归档及 Emby `.mp4` 发布，后两个还发布海报。下载器在已记录的可信运行根目录边界下继续保留锁/scope、归档到数据库恢复及封闭密钥落点。真实签名 CDN 下载及 Emby/Jellyfin 重扫/播放不属于自动验收，继续保持 `NOT_RUN`。
...
- Execution 0016 planned slice: capture the pinned Weibo `mblog.pics` shape in both creator and detail children without modifying `.upstream`, normalize one/many ordinary original pictures into IMAGE/GALLERY Assets, and compose exact refresh, image probe/archive and Emby poster/backdrop/gallery output. Weibo video, retweets, media `page_info`, bounded creator pagination, direct-Sina CDN headers and every live row remain excluded or `NOT_RUN`. / 执行 0016 计划切片：在不修改 `.upstream` 的前提下，于 creator 与 detail child 同时捕获锁定微博 `mblog.pics` 形状，把普通原创单图/多图归一化为 IMAGE/GALLERY Asset，并组合精确刷新、图片探测/归档与 Emby poster/backdrop/gallery 输出。微博视频、转发、媒体 `page_info`、有界 creator 分页、直连新浪 CDN header 及全部真人行继续排除或保持 `NOT_RUN`。
