...
- Remaining acceptance after execution 0007: close the two partial matrices, then add signed-locator refresh, automatic downstream planning, REST/operations and production supervision while preserving the completed offline subscribe → schedule → sync → explicit download → export restart contract. Authorized live platform/CDN and Emby qualification remains Phase 5 evidence. / 执行 0007 之后的剩余验收：先补齐两项部分矩阵，再新增签名 locator 刷新、自动下游规划、REST/运维与生产守护，同时保持已完成的离线“订阅 → 调度 → 同步 → 显式下载 → 导出”重启契约；经授权的真人平台/CDN 与 Emby 验收仍属于 Phase 5 证据。
