...
- Status: execution 0006 delivers the offline-qualified single-host scheduler foundation. Execution 0007 now adds the default-off, license-gated pinned MediaCrawler handler with closed policy v1, manifest v3/receipt v2, attempt-scoped roots, parent-owned heartbeat and process-tree supervision, exact same-transaction SyncRun/ingestion fencing, conservative waiting/failure mapping and four-state cleanup. Strict byte-exact manifest-v2/receipt-v1 readers remain available only for shared normalization/manual ingest; scheduled restart recovery trusts v3. A real local fake-child protocol qualifies the seven-platform forward retry/restart/idempotent-replay chain offline. AC6 deterministic cancellation barriers and AC13's complete failure/secret-sink matrix remain `PARTIAL`. Per-request HTTP spacing, automatic downstream planning, REST, resident supervision and production packaging remain later executions; every live qualification row remains `NOT_RUN`.
- 状态：执行 0006 已交付通过离线验收的单机调度基础。执行 0007 现已新增默认关闭、受许可证约束的锁定版 MediaCrawler handler，包括封闭 policy v1、manifest v3/receipt v2、attempt-scoped 根、父进程拥有的 heartbeat 与进程树监督、精确同事务 SyncRun/导入 fencing、保守 waiting/失败映射，以及四状态清理。严格、逐字节精确的 manifest-v2/receipt-v1 reader 只用于共享归一化/手工导入；定时重启恢复只信任 v3。真实本地 fake-child 协议已离线验收七平台 forward 重试/重启/幂等重放链路。AC6 的确定性取消 barrier 与 AC13 的完整失败/密钥落点矩阵仍为 `PARTIAL`。逐 HTTP 请求间隔、自动下游规划、REST、常驻守护及生产打包仍属于后续执行；全部真人资格行继续为 `NOT_RUN`。
...
- Execution 0008 now freezes the offline successor plan for those two partial criteria: two deterministic cancellation barriers and an exact eleven-failure × three-sink matrix with 33-cell completeness. Its implementation and behavioral verification remain `NOT_RUN`. It deliberately excludes refresh and DAG work so the safety evidence is closed before new network-bearing capability is added. / 执行 0008 现已冻结这两项 partial 的离线继任计划：两个确定性取消 barrier，以及带 33-cell 完整性证明的“11 种失败 × 3 类落点”精确矩阵。其实现与行为验证仍为 `NOT_RUN`。本次有意排除 refresh 与 DAG，先在新增网络能力前关闭安全证据。
- Planned order after execution 0008: implement and verify this closeout; deliver explicit signed-locator refresh with successful/recovery artifact terminal cleanup in execution 0009; then add the durable automatic `sync → download → Emby` DAG in execution 0010. REST/operations and production supervision remain later work. Authorized live platform/CDN and Emby qualification remains Phase 5 evidence. / 执行 0008 之后的计划顺序：先实现并验证本次收口；执行 0009 交付显式签名 locator refresh 及成功/恢复产物终态清理；执行 0010 再新增持久自动 `sync → download → Emby` DAG。REST/运维与生产监督仍属于后续工作；经授权的真人平台/CDN 与 Emby 验收仍属于 Phase 5 证据。
