...
- Status: execution 0009 has resumed with function-first priority. Its migration/backfill, same-transaction provenance ingestion and terminal cleanup focused gates pass; detail refresh, source/Job selection and downloader/CLI wiring are the active gaps. Exhaustive hardening is recorded for follow-up rather than blocking the first usable local path. Execution 0010 has not started; every live qualification row remains `NOT_RUN`.
- 状态：执行 0009 已按功能优先恢复。Migration/backfill、同事务来源导入及终态清理专项通过；detail refresh、来源/Job 选择及下载器/CLI 接线是当前缺口。完整强化矩阵记录为后续工作，不再阻塞首个本地可用路径。执行 0010 尚未开始；全部真人资格行继续为 `NOT_RUN`。
...
- Delivery order: finish a usable execution 0009 refresh/download path, then implement execution 0010's automatic `sync → download → Emby` workflow; run exhaustive hardening and authorized live qualification afterward. / 交付顺序：先完成可用的执行 0009 refresh/download 路径，再实现执行 0010 自动 `sync → download → Emby` 工作流；完整强化与授权真人验收随后执行。
- Execution 0010 functional MVP planning has started in parallel: one durable `pipeline.subscription` coordinator will reuse the existing Job schema, download recovery and Emby publication recovery, so no new migration is planned initially. / 执行 0010 功能 MVP 已并行开始计划：一个持久 `pipeline.subscription` 协调器将复用既有 Job schema、下载恢复与 Emby 发布恢复，因此初期不计划新增 migration。
