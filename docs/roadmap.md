...
- Next delivery order: execution 0012 now adds login hard-parent-death containment, deadline-fenced LoginSession recovery and a local foreground supervisor for the scheduler, subscription worker and pipeline worker. Shutdown cancels and joins MediaCrawler sync, but drains one already-active thread-backed pipeline attempt under heartbeat instead of falsely claiming it can be force-stopped. Authorized per-platform/CDN/real-media-server qualification follows. / 后续交付顺序：执行 0012 现将增加登录父进程硬终止收容、受截止时间 fencing 保护的 LoginSession 回收，以及覆盖 scheduler、订阅 worker 与 pipeline worker 的本地前台监督器。停止时会取消并 join MediaCrawler 同步，但对于已经 active 的线程型 pipeline 尝试，会在 heartbeat 下等待这一项收尾，不虚假声称可以强停；随后再进行经授权的逐平台/CDN/真实媒体服务器验收。
