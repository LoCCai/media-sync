...
- `httpcore`/`httpx` streaming downloads, mandatory bounded `ffprobe` structural validation for video/audio, standard-library XML generation, and optional FFmpeg transformations such as muxing/slideshows.
...
| `media` | Safe download, checksum, mandatory video/audio structural probe, optional FFmpeg transformations / 安全下载、校验、强制音视频结构探测、可选 FFmpeg 转换 | Depend on crawler implementation / 依赖爬虫实现 |
...
任务领取使用租约时间、worker ID 和每次尝试新生成的 fencing token。工作器崩溃后，只有租约过期的任务才能被回收；旧执行不能借用同一 worker ID 后续领取的新租约完成任务。对资产下载而言，精确且已到期的 owner/token 只有在 reclaim 尚未改变 token 时才可续期；renew 与 reclaim 是单胜者 CAS。状态变更使用 CAS，事件序号原子分配，计数在事务内更新。下载流使用绑定 generation 的 `.part`；归档和 Emby 发布分别使用下文的 no-clobber 协议。
...
Execution 0005 runs its application services from the CLI with one SQLite writer. Durable jobs and fencing are designed so a later scheduler and worker processes can reuse the same use cases; the scheduler, REST operations, rate-limit/backoff policy and production packaging are not implemented by this execution. Native platform adapters may progressively replace the restricted bridge; redacted raw envelopes allow re-normalization after either upstream or schema upgrades.

执行 0005 由 CLI 调用应用服务，并使用单 SQLite 写者。持久任务与 fencing 的设计允许后续调度器和 worker 进程复用相同用例；本执行尚未实现调度器、REST 运维、限流/退避策略及生产打包。原生平台适配器可逐步替换受限桥接；经过脱敏的 raw envelope 允许在上游或模型升级后重新归一化。
