...
Forward Bilibili metadata has no CID, so durable identity is deliberately the logical first-page slot rather than a page generation. An unresolved lookup validates the current first CID, but a later same-aid first-CID replacement cannot automatically invalidate already-verified bytes or bump generation. CID-aware replacement belongs with future multi-page discovery. / Bilibili forward 元数据没有 CID，因此持久身份有意采用逻辑首 P 槽，而不是页面 generation。尚未解析时会校验当前首 CID，但后续同 aid 首 CID 替换无法自动使已验证字节失效或提升 generation；CID-aware 替换应与后续多 P 发现一并实现。
