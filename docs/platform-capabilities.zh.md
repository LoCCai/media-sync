...
执行 0055 现已实现鉴权、observation 身份、revision `0008`、append-only replay、仅浏览器确认、有界作者证据 GET 与资格 v3。读取路径在打开短事务前重验 publication/profile 及完整 observation；当前证据独立于历史查询，历史默认 20、最多 50 行，账本总物化行数不超过 limit + 2。远端不确定使历史未知。响应只含本地 ID、时间戳、安全状态与分页边界。当前验证见[投影记录](executions/0055-operator-auth-playback-evidence/evidence-projection/verification.zh.md)。Console v2 与 `/legacy` 仍需 login/session/CSRF 和确认 UI。全部真人行保持 NOT_RUN，可写设置、多配置、保留／破坏性维护与自动扫描仍不属于本阶段。
