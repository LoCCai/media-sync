...
当前工作变更为只读 lookup response 新增仅 matched 的 observation fingerprint、在 Web 中同步该 response 类型、把 Alembic 推进到 `0008_playback_evidence`，并新增 append-only 模型与精确 create-or-replay 仓储原语。`not_found` 结果不能携带该 fingerprint。SQLite 以 immediate 写者预留串行化首次自然身份读取；PostgreSQL 依赖唯一 observation 身份与 savepoint 恢复。当前完整工作树通过 `2868 passed, 22 skipped, 1 warning in 558.19s`；跳过项分别为 3 项 Windows/POSIX、11 项既有 Operation PostgreSQL，以及因未配置测试 URL 而跳过的 8 项新增 PlaybackEvidence PostgreSQL。Web 69 项及 format/check/build、727 文件 Ruff/format、105 个源文件 strict mypy 与 compileall 均通过。真实 PostgreSQL 与 Docker 均未运行；隔离 Author/Job/PlaybackEvidence harness 也不是完整 schema 或部署证据。Console v2 与 `/legacy` 仍未集成 login/session/CSRF，而且不存在经鉴权确认 service/API/UI 或资格 schema v3。因此资格 schema 继续为 v2，`playback_evidence` 能力继续为 `NOT_IMPLEMENTED`，全部真人行继续为 `NOT_RUN`。浏览器可写设置、多配置、保留/破坏性维护与导出后自动扫描仍未实现；后者没有已冻结的后续归属。
