...
执行 0055 为只读 lookup response 新增仅 matched 的 observation fingerprint、在 Web 中同步该 response 类型、把 Alembic 推进到 `0008_playback_evidence`，并新增 append-only 模型与精确 create-or-replay 仓储。当前检查点现已实现并离线验证仅浏览器确认后端：有界 resolve A → 完整唯一 lookup → resolve B 权威检查会在短数据库事务前全部完成；POST 要求精确 Cookie/Origin/CSRF，并拒绝 Bearer-only、混合 Authorization、`Idempotency-Key` 及规范作者 UUID 与不透明 fingerprint 之外的全部请求字段。响应只含安全的证据/作者 ID、服务端时间戳及 replay 状态。当前完整工作树通过 `2941 passed, 22 skipped, 1 warning in 594.72s`；Web 69 项及 format/check/build 与本机可用的全部静态/包/docs 门均通过。真实 PostgreSQL 与 Docker 继续为 `NOT_RUN`；隔离 Author/Job/PlaybackEvidence harness 也不是完整 schema 或部署证据。Console v2 与 `/legacy` 仍未集成 login/session/CSRF 和确认 UI，有界按作者 current/stale 投影及 qualification schema v3 也尚不存在。因此 qualification 继续为 schema v2，其中整体 `playback_evidence` 能力仍为 `NOT_IMPLEMENTED` 且真人状态为空；全部真人行继续为 `NOT_RUN`。浏览器可写设置、多配置、保留/破坏性维护与导出后自动扫描仍未实现；后者没有已冻结的后续归属。
