...
- [ ] `git grep -iE "(set-cookie|cookie:|Authorization:)" -- docs src tests` 只出现脱敏引用、哨兵或夹具——绝无真实 token。
...
- [ ] `[主机]` 干净 clone 演练：全新 `git clone` → 按 `docs/deployment.zh.md` 构建 → 控制台可达 → 一次离线冒烟（`media-sync doctor`）。
