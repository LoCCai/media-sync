...
The normal enabled-supervisor flow already downloads and materializes compatible directories after successful subscription discovery. No manual export or media-server API is required. The Settings page currently reports paths; editable common/per-platform roots remain [next work](executions/0066-login-failure-and-workflow/next-delivery.md). Keep mounts identical across both containers. Updating source is not deployment; coordinate rebuild/recreation separately, and do not restart a deliberately stopped supervisor merely to view the new UI.
...
DY uses real locked JS signing. When localStorage.xmst is absent, it preserves the upstream signer's literal None and sends the identical query, without inventing/fetching another token. Offline consistency does not prove the real platform accepts it. Integer status_code0 is this release's conservative response subset. DY/KS pasted self validation remains unavailable; an existing Cookie source is not a newly implemented validator. DY/KS/Tieba target-profile lookup neither proves nor updates the current self authentication.

Tieba avatars accept only the fixed source-backed gss0.bdstatic.com path with the same creator portrait, optionally retaining that response's timestamp. Unknown img_url is not fetched; missing imagery uses the source-proven fixed fallback. Existing2MiB/pixel/deadline/no-redirect/no-Cookie/private-DNS rejection and same-origin PNG rules remain. Avatar failure retains nicknames/old avatars; refresh does not move archives or rename exports. XHS profiles and DY/KS/Zhihu avatars remain pending. No new environment/dependency/schema/media-server configuration; no production deployment, canary retry or supervisor restart this round.

## Tieba pasted Cookie and KS/Zhihu nicknames (0063)

After separately updating/rebuilding the service, Accounts can validate and save a Tieba request Cookie header through the same protected paste dialog. It performs one HTTPS self request, not QR or crawling, then saves only after strict positive remote identity proof. Nonzero/unknown codes, malformed identity and guest responses keep the previous credential. This release accepts integer no=0, positive uint64 integer id and modern tb.1. portraits; legacy/IP portraits or unproven alternative response types are not claimed. A local BDUSS marker alone never authenticates. DY/KS pasted self validation remains unavailable.

In Subscriptions, an eligible authenticated saved-session or existing Cookie account can query KS/Zhihu nicknames once on input blur/Enter. KS takes its exact opaque user ID (maximum128 ASCII characters), Zhihu its url_token (maximum255, excluding dot/dotdot), not a whole URL or numeric-ID conversion. The profile response must carry the same identity. KS only observes a creator; it does not validate or change the logged-in self account. Zhihu first checks its self endpoint, then reads one exact profile page. No posts, media, QR or automatic retries are requested. Standalone lookup does not require capture's full-history acknowledgement.

Both new platforms currently return nickname only: avatar retrieval remains unimplemented rather than fetching unproven URLs. Bili/Weibo retain their existing optional-avatar support. Failed or stale refreshes preserve prior observations; a nickname refresh never renames existing archive/export paths. Saved Cookie secrets remain private and must not be sent in issues. No new migration, environment setting or media-server connection is required. Current Linux and real platform qualification remain separate; source publication does not authorize deployment, a live retry or supervisor restart.
...
CLI may report `status=unknown`, `error_code=ingestion_state_unconfirmed` and `observed_error_code=content_ownership_conflict` when it cannot confirm the durable failure state. This means inspect the exact Run and service/storage health before another attempt; it is neither a confirmed terminal failure nor permission to retry. No new database migration or configuration is required. This increment does not implement Bili dynamic attachments, deploy an image, authorize a live retry or restart supervisor. See [0061 verification](executions/0061-bili-dynamic-authority/verification.md) for final gates and exclusions.

## Weibo creator profiles (0060)

After selecting an authenticated Weibo saved-session or Cookie account, enter a canonical numeric creator UID and finish the input (blur/Enter) for one automatic lookup. It performs one login-config check before one exact creator-info request; no posts, comments or downloads are requested. Nickname is platform-observed, local alias is optional, and the source homepage is fixed to https://weibo.com/u/UID. Profile refresh never renames an existing author/archive/export path. It does not require an Emby/Jellyfin connection or full-history acknowledgement; the separate Weibo capture policy still does require the latter.

Avatars are optional same-origin PNGs, with isolated bounded download/decode and platform-bound HTTPS CDN rules. The narrow Sina crop/size paths and optional profile_image_url fallback are synthetic design contracts, not proven live URL shapes. Unknown, signed, redirected or unsupported image forms are not fetched; nickname and prior avatar remain usable. Executions0063/0064 also add KS/Zhihu/DY/Tieba nicknames and Tieba avatars; XHS profiles and DY/KS/Zhihu avatars remain pending. Execution0060 added no dependencies or database migration. See [0060 verification](executions/0060-weibo-creator-profile/verification.md); deploy and live-qualify separately, without automatically restarting supervisor.

## Bounded Bili subscriptions (0059)

New Bili scheduled captures and dry-runs use resumable ordinary-upload units without the old unbounded-history checkbox. Each unit verifies at most min(max_items,30) details and makes at most two author-list attempts. Browser/auth initialization and up to two WBI signing-key reads are separate. Head refresh and historical scanning alternate; the private checkpoint retains unfinished identities across successful runs. Older cursors start with no assumed coverage, while malformed or mismatched new-version cursors reject safely rather than being silently reset.

The subscription detail view distinguishes per-unit progress, page-change restarts and source-end observations. None claims that all author history or dynamic attachments have been captured. A small max_items is **not** a download bound: the pipeline can process the subscription's prior pending assets. Keep supervisor stopped for a separately authorized capture-only canary; source publication does not authorize deployment, retry or background downloads. No new database migration or dependency is introduced. Preserve the state database and credentials as before. See [0059 verification](executions/0059-bili-bounded-capture/verification.md) for offline proof and live exclusions.

## Pasted Cookie login (0058)

The Accounts page now has an explicit **paste → remote self-check → private save** workflow for Bili, XHS, WB, Zhihu and Tieba added in0063. Paste only the browser request Cookie header value, not `Cookie:`, Set-Cookie attributes or JSON exports; never send credentials to chat or attach them to a support report. Maximum input is 16 KiB and 128 unique pairs. Values containing equals signs and valid outer quotes are preserved. DY and KS still lack a qualified remote identity validator and are explicitly unavailable, not considered authenticated from local Cookie markers. Tieba's conservative accepted response subset is documented in this page's0063 section.
