...
The local function-first path is implemented through execution 0016 implementation commit `a77ca74`. In addition to explicit QR/session recovery and the foreground `scheduler supervise` chain, four frozen platform shapes now reach qualified offline media output: execution 0013's Bilibili logical-first-page single-progressive video, execution 0014's Kuaishou ordinary single video plus optional cover, execution 0015's ordinary numeric-ID Douyin single video plus optional cover with empty image/music fields, and execution 0016's ordinary original Weibo static-image IMAGE/GALLERY shape. The Weibo slice captures ordered `mblog.pics` in both creator and numeric-ID detail children without modifying `.upstream`, restricts media to query-free `.jpg/.jpeg/.png/.webp` `i1.wp.com` locators that embed a `sinaimg.cn` source host, and composes exact Account/Subscription refresh, mock image transfer, SHA-256 archive and idempotent Emby poster/backdrop/gallery/NFO/source publication. The execution 0016 focused gate passes 388 tests in 125.73 seconds; the complete suite passes 1251 tests with one Windows-inapplicable POSIX mode-bit test skipped in 359.38 seconds. No coverage run is claimed. Real login, creator scan, detail request, CDN transfer, platform bytes and Emby/Jellyfin server validation all remain `NOT_RUN`. This is not seven-platform complete download, an auto-restarting daemon, Windows Service/systemd unit, forced synchronous-thread cancellation or cross-host HA. Exact evidence is in [`docs/executions/0016-weibo-image-gallery/verification.md`](docs/executions/0016-weibo-image-gallery/verification.md).

本地功能优先链路已实现到执行 0016 实现提交 `a77ca74`。除显式 QR/会话回收及前台 `scheduler supervise` 全链外，四个冻结的平台形状现已产生通过验收的离线媒体输出：执行 0013 的 Bilibili 逻辑首 P 单 progressive 视频、执行 0014 的快手普通单视频与可选封面、执行 0015 的普通 numeric-ID 抖音单视频与可选封面（图片与音乐字段为空），以及执行 0016 的普通原创微博静态图片 IMAGE/GALLERY 形状。微博切片在不修改 `.upstream` 的前提下，于 creator 与 numeric-ID detail child 捕获有序 `mblog.pics`，只接受内嵌 `sinaimg.cn` 源 host、无 query 的 `.jpg/.jpeg/.png/.webp` `i1.wp.com` locator，并组合精确 Account/Subscription 刷新、mock 图片传输、SHA-256 归档及幂等 Emby poster/backdrop/gallery/NFO/source 发布。执行 0016 专项门禁通过 388 项，耗时 125.73 秒；完整套件通过 1251 项、跳过一项 Windows 不适用的 POSIX mode-bit 测试，耗时 359.38 秒。不宣称运行过覆盖率。真人登录、作者扫描、detail 请求、CDN 传输、真实平台字节及 Emby/Jellyfin 服务器验收全部保持 `NOT_RUN`。这不代表七平台完整下载，也不是自动重启 daemon、Windows Service/systemd 服务，不提供同步线程强停或跨主机 HA。准确证据位于 [`docs/executions/0016-weibo-image-gallery/verification.md`](docs/executions/0016-weibo-image-gallery/verification.md)。

- Implemented offline / 已实现离线：the four frozen shapes above, including Weibo ordinary-original static IMAGE/GALLERY discovery, refresh, archive and Emby filesystem publication / 上述四个冻结形状，包括微博普通原创静态 IMAGE/GALLERY 的发现、刷新、归档与 Emby 文件系统发布。
- Still pending or unclaimed / 仍待实现或未验收：other platform/media shapes across the larger seven-platform goal, including Weibo video, retweets, media `page_info` and animation; bounded creator pagination; live platform/CDN/real-byte/media-server qualification / 七平台大目标中的其他平台与媒体形状，包括微博视频、转发、媒体 `page_info` 与动图；有界作者分页；真人平台/CDN/真实字节/媒体服务器验收。
...
The second through fifth commands are the execution 0013 Bilibili, execution 0014 Kuaishou, execution 0015 Douyin and execution 0016 Weibo composition tests. They use synthetic metadata, fake detail results, mock transport bytes and controlled media validation; they prove local pipeline contracts, not a real CDN, real platform bytes, FFmpeg against platform video or a running Emby/Jellyfin server. / 第二至第五条命令分别是执行 0013 的 Bilibili、执行 0014 的快手、执行 0015 的抖音及执行 0016 的微博组合测试。它们使用合成元数据、fake detail 结果、mock transport 字节及受控媒体校验；只证明本地流水线契约，不证明真实 CDN、真实平台字节、针对平台视频的 FFmpeg 或运行中的 Emby/Jellyfin 服务器。
...
MediaCrawler-discovered assets persist only the stable, secret-free `adapter_refresh` locator. Execution 0009 resolves it lazily from the exact current Subscription source when `asset download` or the pipeline receives both MediaCrawler enable/license switches. Manual selection also accepts `--subscription-id`; XHS accepts the one-note detail reference described above. Before any child Job or Asset lifecycle write, preflight validates the pinned lock/checkout/Python runtime and an actually launchable mandatory `ffprobe`; invalid non-empty configuration fails with zero child lifecycle side effects. The signed result is used in memory by the safe downloader and is never written back to SQLite.
