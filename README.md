...
The local function-first path is implemented through execution 0025 implementation commit `fe45abc`. In addition to explicit QR/session recovery and the foreground `scheduler supervise` chain, twelve frozen media shapes across all seven platforms now have focused offline evidence: execution 0013's Bilibili logical-first-page single-progressive video, execution 0014's Kuaishou ordinary single video plus optional cover, execution 0015's ordinary numeric-ID Douyin single video plus optional cover with empty image/music fields, execution 0016's ordinary-original Weibo static IMAGE/GALLERY, execution 0017's ordinary `type="normal"` XHS static IMAGE/GALLERY, execution 0018's ordinary `type="video"` XHS single playable video with zero or one static IMAGE artwork, execution 0019's ordinary Zhihu creator answer with exactly one static IMAGE, executions 0020–0022's compatible one-/two-/3–64-static-image Tieba first-floor shapes, execution 0023's compatible 2–64-page Bilibili multipart progressive upload, and execution 0024's compatible single-/2–64-page Bilibili DASH video/audio lifecycle. Execution 0025 hardens that existing DASH shape; it does not add a thirteenth shape.

Execution 0024 established strict Bilibili detail protocol v5, closed DASH selection, generation-scoped component stores, bounded fixed-argv ffmpeg `-c copy`, final probing and final-only archive publication. Execution 0025 now tries each selected DASH video's and optional audio's primary URL followed by at most eight backups, independently and in source order. DNS, timeout, transport, interruption, HTTP-status and partial-Range incompatibility outcomes may advance to the next candidate; network-policy, redirect/header/encoding, chunk/size, filesystem, probe and mux failures still fail immediately. Cross-candidate append requires the same offset, total length and validator, mixed failures preserve partial bytes, and bounded discard/restart occurs only after the complete candidate pass rejects the partial. Signed candidates and the winning index remain ephemeral. The real offline H.264+AAC composition exercises video-primary `503` and audio-primary `403`, reaches independent backups, and publishes an Emby MP4 containing both streams. Final focused regression passes `466 passed in 66.96s`; the complete suite passes `1790 passed, 1 skipped in 331.33s`; all quality/build/docs/upstream/diff gates pass. Implementation `fe45abc` is pushed and reconciled. Real login, authenticated API/CDN and Emby/Jellyfin server validation remain `NOT_RUN`. Exact evidence is in [`docs/executions/0025-bilibili-dash-cdn-failover/verification.md`](docs/executions/0025-bilibili-dash-cdn-failover/verification.md).

本地功能优先链路已实现到执行 0025 实现提交 `fe45abc`。除显式 QR/会话回收及前台 `scheduler supervise` 全链外，覆盖全部七个平台的十二个冻结媒体形状现已有专项离线证据：执行 0013 的 Bilibili 逻辑首 P 单 progressive 视频、执行 0014 的快手普通单视频与可选封面、执行 0015 的普通 numeric-ID 抖音单视频与可选封面（图片与音乐字段为空）、执行 0016 的普通原创微博静态 IMAGE/GALLERY、执行 0017 的普通 `type="normal"` 小红书静态 IMAGE/GALLERY、执行 0018 的普通 `type="video"` 小红书单个可播放视频与零或一张静态 IMAGE 封面、执行 0019 的知乎作者普通回答精确一张静态 IMAGE、执行 0020–0022 的贴吧兼容单图/双图/3–64 图首楼形状、执行 0023 的 Bilibili 兼容 2–64 分 P progressive 投稿，以及执行 0024 的 Bilibili 兼容单 P/2–64 分 P DASH 音视频生命周期。执行 0025 加固的是既有 DASH 形状，不会把冻结媒体形状数误增为十三个。

执行 0024 已建立严格 Bilibili 详情协议 v5、封闭 DASH 选择、generation-scoped 组件 store、固定参数有界 ffmpeg `-c copy`、成品探测及仅成品归档发布。执行 0025 现让每个已选 DASH 视频及可选音频分别按来源顺序尝试主地址与最多八个备用地址。DNS、timeout、传输、中断、HTTP 状态及 partial Range 不兼容可推进下一候选；网络策略、重定向/header/encoding、chunk/size、文件系统、探测与合并失败仍立即关闭。跨候选追加要求 offset、总长度及 validator 完全相同；混合失败保留 partial，只有完整候选轮次拒绝 partial 后才有界丢弃/restart。签名候选与胜出序号保持瞬态。真实离线 H.264+AAC 组合让视频主地址返回 `503`、音频主地址返回 `403`，分别切换到备用地址，并发布同时含音视频流的 Emby MP4。最终专项回归通过 `466 passed in 66.96s`；完整套件通过 `1790 passed, 1 skipped in 331.33s`；全部质量/构建/文档/上游/diff 门通过。实现 `fe45abc` 已推送并核对。真人登录、登录态 API/CDN 与 Emby/Jellyfin 服务器验收保持 `NOT_RUN`。准确证据位于 [`docs/executions/0025-bilibili-dash-cdn-failover/verification.md`](docs/executions/0025-bilibili-dash-cdn-failover/verification.md)。

- Implemented with focused offline evidence / 已实现并有专项离线证据：the twelve frozen shapes above, including compatible Bilibili single-/2–64-page progressive and DASH publication, bounded Zhihu/Tieba discovery and exact canonical refresh/archive/Emby output / 上述十二个冻结形状，包括兼容 Bilibili 单 P/2–64 分 P progressive 与 DASH 发布、知乎/贴吧有界发现及精确 canonical 刷新/归档/Emby 输出。
- Still pending or unclaimed / 仍待实现或未验收：Bilibili segmented `durl`, subtitles/danmaku, progressive backup-CDN failover, CDN ranking/cache, FLV and pages above 64; Tieba galleries above 64 and mixed/rich/reply media; Zhihu multiple images/articles/zvideo; other shapes in the larger seven-platform goal; and every live platform/CDN/real-byte/media-server row / Bilibili 分段 `durl`、字幕/弹幕、progressive 备用 CDN 故障切换、CDN 排序/缓存、FLV 与超过 64 个分 P；贴吧超过 64 张的 gallery 与混合/富内容/回复媒体；知乎多图/文章/zvideo；七平台大目标中的其他形状；以及全部真人平台/CDN/真实字节/媒体服务器行。
