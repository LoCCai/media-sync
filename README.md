...
The local function-first path is implemented through execution 0024 implementation commit `12314b9`. In addition to explicit QR/session recovery and the foreground `scheduler supervise` chain, twelve frozen media shapes across all seven platforms now have focused offline evidence: execution 0013's Bilibili logical-first-page single-progressive video, execution 0014's Kuaishou ordinary single video plus optional cover, execution 0015's ordinary numeric-ID Douyin single video plus optional cover with empty image/music fields, execution 0016's ordinary-original Weibo static IMAGE/GALLERY, execution 0017's ordinary `type="normal"` XHS static IMAGE/GALLERY, execution 0018's ordinary `type="video"` XHS single playable video with zero or one static IMAGE artwork, execution 0019's ordinary Zhihu creator answer with exactly one static IMAGE, executions 0020–0022's compatible one-/two-/3–64-static-image Tieba first-floor shapes, execution 0023's compatible 2–64-page Bilibili multipart progressive upload, and execution 0024's compatible single-/2–64-page Bilibili DASH video/audio lifecycle.

Execution 0024 upgrades strict Bilibili detail protocol to v5 and requests the exact CID with WBI signing, `qn=127`, `fourk=1` and `fnval=4048`. It selects the highest supported video quality, prefers AVC then HEV then AV1 at equal quality, and selects the highest pinned ordinary/Dolby/Hi-Res audio while allowing silent video. Signed component URLs remain ephemeral. Generation-scoped components use strict resume and structural probes, a bounded fixed-argv ffmpeg `-c copy` mux, a final probe and final-only SHA-256 archive publication; failed muxes retain verified components and published finals can recover without detail/DNS/HTTP/ffmpeg. A real offline H.264+AAC composition reaches Emby with both streams and no signed target in retained state. Final focused regression passes `456 passed in 66.47s`; the complete suite passes `1780 passed, 1 skipped in 333.43s`; all quality/build/docs/upstream/diff gates pass. Implementation `12314b9` is pushed and reconciled. Real login, authenticated API/CDN and Emby/Jellyfin server validation remain `NOT_RUN`. Exact evidence is in [`docs/executions/0024-bilibili-dash-mux/verification.md`](docs/executions/0024-bilibili-dash-mux/verification.md).

本地功能优先链路已实现到执行 0024 实现提交 `12314b9`。除显式 QR/会话回收及前台 `scheduler supervise` 全链外，覆盖全部七个平台的十二个冻结媒体形状现已有专项离线证据：执行 0013 的 Bilibili 逻辑首 P 单 progressive 视频、执行 0014 的快手普通单视频与可选封面、执行 0015 的普通 numeric-ID 抖音单视频与可选封面（图片与音乐字段为空）、执行 0016 的普通原创微博静态 IMAGE/GALLERY、执行 0017 的普通 `type="normal"` 小红书静态 IMAGE/GALLERY、执行 0018 的普通 `type="video"` 小红书单个可播放视频与零或一张静态 IMAGE 封面、执行 0019 的知乎作者普通回答精确一张静态 IMAGE、执行 0020–0022 的贴吧兼容单图/双图/3–64 图首楼形状、执行 0023 的 Bilibili 兼容 2–64 分 P progressive 投稿，以及执行 0024 的 Bilibili 兼容单 P/2–64 分 P DASH 音视频生命周期。

执行 0024 把严格 Bilibili 详情协议升级到 v5，以 WBI 签名、`qn=127`、`fourk=1` 与 `fnval=4048` 请求精确 CID；选择最高受支持视频画质，同画质依次偏好 AVC、HEV、AV1，并按锁定顺序选择最高普通/杜比/Hi-Res 音频，同时允许无声视频。签名组件 URL 保持瞬态。Generation-scoped 组件使用严格续传与结构探测，经过固定参数、有界的 ffmpeg `-c copy` 合并及成品探测后，仅成品进入 SHA-256 归档；合并失败保留已验证组件，已发布成品可在不执行 detail/DNS/HTTP/ffmpeg 的情况下恢复。真实离线 H.264+AAC 组合已以双流成品进入 Emby，且保留状态中无签名 target。最终专项回归通过 `456 passed in 66.47s`；完整套件通过 `1780 passed, 1 skipped in 333.43s`；全部质量/构建/文档/上游/diff 门通过。实现 `12314b9` 已推送并核对。真人登录、登录态 API/CDN 与 Emby/Jellyfin 服务器验收保持 `NOT_RUN`。准确证据位于 [`docs/executions/0024-bilibili-dash-mux/verification.md`](docs/executions/0024-bilibili-dash-mux/verification.md)。
...
- Still pending or unclaimed / 仍待实现或未验收：Bilibili segmented `durl`, subtitles/danmaku, backup-CDN failover, FLV and pages above 64; Tieba galleries above 64 and mixed/rich/reply media; Zhihu multiple images/articles/zvideo; other shapes in the larger seven-platform goal; and every live platform/CDN/real-byte/media-server row / Bilibili 分段 `durl`、字幕/弹幕、备用 CDN 故障切换、FLV 与超过 64 个分 P；贴吧超过 64 张的 gallery 与混合/富内容/回复媒体；知乎多图/文章/zvideo；七平台大目标中的其他形状；以及全部真人平台/CDN/真实字节/媒体服务器行。
