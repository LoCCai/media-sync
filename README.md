...
The local function-first path is implemented through execution 0020 implementation commit `8a0e935`. In addition to explicit QR/session recovery and the foreground `scheduler supervise` chain, eight frozen media shapes across all seven platforms now have focused offline evidence: execution 0013's Bilibili logical-first-page single-progressive video, execution 0014's Kuaishou ordinary single video plus optional cover, execution 0015's ordinary numeric-ID Douyin single video plus optional cover with empty image/music fields, execution 0016's ordinary-original Weibo static IMAGE/GALLERY, execution 0017's ordinary `type="normal"` XHS static IMAGE/GALLERY, execution 0018's ordinary `type="video"` XHS single playable video with zero or one static IMAGE artwork, execution 0019's ordinary Zhihu creator answer with exactly one static IMAGE, and execution 0020's ordinary Tieba creator thread first floor with exactly one static IMAGE.

Execution 0020 installs an integration-owned shim only after verified MediaCrawler import. It captures one current integer type-3 first-floor image before the pinned Tieba extractor/store path drops locators, binds it to the exact returned object across gather-child/parent-store tasks, and hardens scheduled creator execution to exact successful Subscription `max_items`. The normalized row remains ARTICLE with one position-zero `<note_id>:image:0` IMAGE; only query-free identity is durable, while exact persisted thread authority drives numeric-ID detail and credential-free `MediaRequestProfile.DEFAULT` signed refresh. Tieba IMAGE downloads automatically require bounded static structural qualification: qualified JPEG/PNG/WebP pass, while GIF/APNG/animated WebP/AVIF fail; this is not a complete pixel decoder. The focused regression passes `368 passed in 41.18s`, the complete suite passes `1650 passed, 1 skipped in 310.82s`, and all quality/build/upstream/audit gates pass. Implementation commit `8a0e935` is pushed and reconciled; this change is the bilingual documentation closeout, with its self-referential SHA intentionally left to Git history. There is no retained redacted real Tieba fixture, and real login, creator/detail, future CDN bytes and Emby/Jellyfin server validation remain `NOT_RUN`. Exact evidence is in [`docs/executions/0020-tieba-first-floor-image/verification.md`](docs/executions/0020-tieba-first-floor-image/verification.md).

本地功能优先链路已实现到执行 0020 实现提交 `8a0e935`。除显式 QR/会话回收及前台 `scheduler supervise` 全链外，覆盖全部七个平台的八个冻结媒体形状现已有专项离线证据：执行 0013 的 Bilibili 逻辑首 P 单 progressive 视频、执行 0014 的快手普通单视频与可选封面、执行 0015 的普通 numeric-ID 抖音单视频与可选封面（图片与音乐字段为空）、执行 0016 的普通原创微博静态 IMAGE/GALLERY、执行 0017 的普通 `type="normal"` 小红书静态 IMAGE/GALLERY、执行 0018 的普通 `type="video"` 小红书单个可播放视频与零或一张静态 IMAGE 封面、执行 0019 的知乎作者普通回答精确一张静态 IMAGE，以及执行 0020 的贴吧作者普通主题首楼精确一张静态 IMAGE。

执行 0020 只在 MediaCrawler 校验导入后安装集成自有 shim。它在锁定贴吧 extractor/store 路径丢弃 locator 前捕获一个当前整数 type-3 首楼图片，把捕获绑定到跨 gather-child/父 store 的精确返回对象，并把 scheduled creator 加固为精确成功的 Subscription `max_items`。归一化行继续为 ARTICLE，并拥有 position 0 的唯一 `<note_id>:image:0` IMAGE；只有无 query 身份持久化，精确持久主题权限驱动 numeric-ID detail 与无凭据 `MediaRequestProfile.DEFAULT` 签名刷新。贴吧 IMAGE 下载自动要求有界静态结构资格校验：合格 JPEG/PNG/WebP 通过，GIF/APNG/animated WebP/AVIF 失败；该门不是完整像素解码器。专项回归通过 `368 passed in 41.18s`，完整套件通过 `1650 passed, 1 skipped in 310.82s`，全部质量/构建/上游/审计门通过。实现提交 `8a0e935` 已推送并核对；本次变更即双语文档收尾，自引用 SHA 有意只保留在 Git 历史中。当前没有保留的真实脱敏贴吧夹具；真人登录、creator/detail、未来 CDN 字节及 Emby/Jellyfin 服务器验收保持 `NOT_RUN`。准确证据位于 [`docs/executions/0020-tieba-first-floor-image/verification.md`](docs/executions/0020-tieba-first-floor-image/verification.md)。

- Implemented with focused offline evidence / 已实现并有专项离线证据：the eight frozen shapes above, including bounded Zhihu-answer and Tieba-thread discovery, exact canonical refresh, archive and Emby filesystem publication for one static image on each / 上述八个冻结形状，包括知乎回答与贴吧主题有界发现、精确 canonical 刷新，以及各自单静态图归档与 Emby 文件系统发布。
- Still pending or unclaimed / 仍待实现或未验收：Tieba gallery and other rich/media types; Zhihu multiple images/articles/zvideo; other shapes in the larger seven-platform goal; and every live platform/CDN/real-byte/media-server row / 贴吧 gallery 与其他富内容/媒体类型；知乎多图/文章/zvideo；七平台大目标中的其他形状；以及全部真人平台/CDN/真实字节/媒体服务器行。
...
The second through ninth commands are the execution 0013 Bilibili, execution 0014 Kuaishou, execution 0015 Douyin, execution 0016 Weibo, execution 0017 XHS static, execution 0018 XHS playable-video, execution 0019 Zhihu answer-image and execution 0020 Tieba first-floor-image compositions. They use synthetic metadata, fake detail results, mock transport bytes and controlled or production media gates; execution 0018 separately validates an embedded real H.264 MP4 with production `FFprobeMediaProbe`, while executions 0019 and 0020 qualify bounded JPEG/PNG/WebP structures and reject tested GIF/APNG/animated WebP/AVIF payloads. The static gate is not a complete pixel decoder. These commands prove local pipeline contracts, not a real creator/feed/detail request, CDN/platform bytes or a running Emby/Jellyfin server. No retained redacted real Zhihu or Tieba source fixture is claimed. / 第二至第九条命令分别是执行 0013 的 Bilibili、执行 0014 的快手、执行 0015 的抖音、执行 0016 的微博、执行 0017 的小红书静态内容、执行 0018 的小红书可播放视频、执行 0019 的知乎回答图片及执行 0020 的贴吧首楼图片组合测试。它们使用合成元数据、fake detail 结果、mock transport 字节及受控或生产媒体门；执行 0018 另以生产 `FFprobeMediaProbe` 验证内嵌真实 H.264 MP4，执行 0019 与 0020 则校验有界 JPEG/PNG/WebP 结构并拒绝已测试的 GIF/APNG/animated WebP/AVIF 字节。静态门不是完整像素解码器。这些命令只证明本地流水线契约，不证明真实 creator/feed/detail 请求、CDN/平台字节或运行中的 Emby/Jellyfin 服务器，也不宣称存在保留的真实脱敏知乎或贴吧来源夹具。
