...
The core foundation, credential-safe MediaCrawler bridge, offline-qualified media pipeline, execution 0006 single-host scheduler, and execution 0007 opt-in MediaCrawler scheduled handler are runnable. New scheduled attempts write manifest v3/receipt v2 artifacts beneath unique attempt roots; sealed legacy manifest v2/receipt v1 evidence remains byte-exact, read-only and available only through strict shared normalization/manual-ingest compatibility, never scheduled restart recovery. The trusted parent owns lease heartbeat and process-tree supervision, and exact owner/token/unexpired guards fence every SyncRun and ingestion/checkpoint transaction. A real local fake-child protocol proves the complete seven-platform subscribe → tick → write/load → guarded ingest → retry/restart → idempotent replay chain offline. Execution 0008 now closes execution 0007 AC6/AC13 as successor offline evidence with both deterministic cancellation barriers and an exact eleven-failure × three-sink security matrix; the full gate passed 837 tests with one Windows-inapplicable skip and 79% branch-aware coverage. This does not qualify live behavior. Signed-locator refresh, the durable automatic `sync → download → Emby` DAG, per-request HTTP spacing, REST/Docker operations, and every user-authorized live platform/CDN or Emby/Jellyfin row remain outstanding. Requirements, capability truth, progress, and exact verification evidence are tracked in [`docs/`](docs/README.md).

核心基线、安全凭据的 MediaCrawler 桥接、通过离线验收的媒体流水线、执行 0006 的单机调度器，以及执行 0007 显式启用的 MediaCrawler 定时 handler 均已可运行。新的定时 attempt 会在唯一 attempt 根下写入 manifest v3/receipt v2；已密封的 legacy manifest v2/receipt v1 证据保持逐字节精确、只读，并且只能通过严格的共享归一化/手工导入兼容路径读取，不能用于定时重启恢复。可信父进程拥有 lease heartbeat 与进程树监督权，每个 SyncRun 及导入/checkpoint 事务都受精确 owner/token/未过期 guard 保护。本地真实 fake-child 协议已离线证明七个平台完整的“订阅 → tick → 写入/读取 → 受保护导入 → 重试/重启 → 幂等重放”链路。执行 0008 现以继任离线证据关闭执行 0007 的 AC6/AC13，覆盖两个确定性取消 barrier 与精确“11 种失败 × 3 类落点”安全矩阵；完整门禁通过 837 项测试、1 项 Windows 不适用的 skip，分支感知覆盖率为 79%。这不代表真人行为已验收。签名 locator refresh、持久自动 `sync → download → Emby` DAG、逐 HTTP 请求间隔、REST/Docker 运维，以及全部需要用户授权的真人平台/CDN 或 Emby/Jellyfin 行仍待完成。需求、真实能力、进展及准确验证证据均保存在 [`docs/`](docs/README.md)。
