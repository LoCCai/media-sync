...
The local function-first path is implemented through execution 0021 implementation commit `e0fb8d5`. In addition to explicit QR/session recovery and the foreground `scheduler supervise` chain, nine frozen media shapes across all seven platforms now have focused offline evidence: execution 0013's Bilibili logical-first-page single-progressive video, execution 0014's Kuaishou ordinary single video plus optional cover, execution 0015's ordinary numeric-ID Douyin single video plus optional cover with empty image/music fields, execution 0016's ordinary-original Weibo static IMAGE/GALLERY, execution 0017's ordinary `type="normal"` XHS static IMAGE/GALLERY, execution 0018's ordinary `type="video"` XHS single playable video with zero or one static IMAGE artwork, execution 0019's ordinary Zhihu creator answer with exactly one static IMAGE, execution 0020's ordinary Tieba creator thread first floor with exactly one static IMAGE, and execution 0021's compatible exact-two-static-image Tieba first floor.

Execution 0021 extends the verified Tieba shim without changing execution 0020's v1 single-image field. An exact ordered pair uses a separate v2 private field; dual claims, duplicate query-free identities, three-or-more images and other first-floor content types fail closed. The row remains ARTICLE and materializes `<note_id>:image:0` plus `<note_id>:image:1`. Lazy refresh binds both persisted identities, requires the complete current ordered gallery for either position, rejects missing/reordered/replaced galleries and returns only a newly validated signed URL under credential-free `MediaRequestProfile.DEFAULT`. Two exact JPEG/PNG downloads pass the production static gate, reach distinct SHA-256 archives and publish deterministic Emby poster/backdrop/two-file-gallery/body/NFO/source output; query-only replay performs zero new detail/DNS/HTTP/archive/export work. The focused regression passes `413 passed in 44.50s`, the complete suite passes `1668 passed, 1 skipped in 314.72s`, and all quality/build/upstream/audit gates pass. Implementation `e0fb8d5` is pushed and reconciled. There is no retained real Tieba fixture; real login, creator/detail, future CDN bytes and Emby/Jellyfin server validation remain `NOT_RUN`. Exact evidence is in [`docs/executions/0021-tieba-first-floor-two-image-gallery/verification.md`](docs/executions/0021-tieba-first-floor-two-image-gallery/verification.md).

本地功能优先链路已实现到执行 0021 实现提交 `e0fb8d5`。除显式 QR/会话回收及前台 `scheduler supervise` 全链外，覆盖全部七个平台的九个冻结媒体形状现已有专项离线证据：执行 0013 的 Bilibili 逻辑首 P 单 progressive 视频、执行 0014 的快手普通单视频与可选封面、执行 0015 的普通 numeric-ID 抖音单视频与可选封面（图片与音乐字段为空）、执行 0016 的普通原创微博静态 IMAGE/GALLERY、执行 0017 的普通 `type="normal"` 小红书静态 IMAGE/GALLERY、执行 0018 的普通 `type="video"` 小红书单个可播放视频与零或一张静态 IMAGE 封面、执行 0019 的知乎作者普通回答精确一张静态 IMAGE、执行 0020 的贴吧作者普通主题首楼精确一张静态 IMAGE，以及执行 0021 兼容扩展的贴吧首楼精确两张静态 IMAGE。

执行 0021 在不改变执行 0020 v1 单图字段的前提下扩展校验贴吧 shim。精确有序双图使用独立 v2 私有字段；双重声明、重复无 query 身份、三张及以上图片及其他首楼内容类型均关闭失败。归一化行保持 ARTICLE，并物化 `<note_id>:image:0` 与 `<note_id>:image:1`。惰性刷新绑定两个持久身份，为任一 position 要求当前完整有序 gallery，拒绝缺图、重排及替换，并只以无凭据 `MediaRequestProfile.DEFAULT` 返回新校验签名 URL。两次精确 JPEG/PNG 下载通过生产静态门，进入两个 SHA-256 归档，并确定性发布 Emby poster/backdrop/双文件 gallery/body/NFO/source；仅 query 变化的重放不新增 detail/DNS/HTTP/archive/export 工作。专项回归通过 `413 passed in 44.50s`，完整套件通过 `1668 passed, 1 skipped in 314.72s`，全部质量/构建/上游/审计门通过。实现 `e0fb8d5` 已推送并核对。当前没有保留的真实贴吧夹具；真人登录、creator/detail、未来 CDN 字节及 Emby/Jellyfin 服务器验收保持 `NOT_RUN`。准确证据位于 [`docs/executions/0021-tieba-first-floor-two-image-gallery/verification.md`](docs/executions/0021-tieba-first-floor-two-image-gallery/verification.md)。

- Implemented with focused offline evidence / 已实现并有专项离线证据：the nine frozen shapes above, including bounded Zhihu-answer and Tieba-thread discovery, exact canonical refresh, and archive/Emby publication for compatible one- or exact-two-image Tieba first floors / 上述九个冻结形状，包括知乎回答与贴吧主题有界发现、精确 canonical 刷新，以及兼容单图或精确双图贴吧首楼的归档/Emby 发布。
- Still pending or unclaimed / 仍待实现或未验收：Tieba three-or-more-image galleries and other rich/media types; Zhihu multiple images/articles/zvideo; other shapes in the larger seven-platform goal; and every live platform/CDN/real-byte/media-server row / 贴吧三张及以上 gallery 与其他富内容/媒体类型；知乎多图/文章/zvideo；七平台大目标中的其他形状；以及全部真人平台/CDN/真实字节/媒体服务器行。
...
The second through ninth commands are the execution 0013 Bilibili, execution 0014 Kuaishou, execution 0015 Douyin, execution 0016 Weibo, execution 0017 XHS static, execution 0018 XHS playable-video, execution 0019 Zhihu answer-image and executions 0020–0021 Tieba first-floor-image compositions. They use synthetic metadata, fake detail results, mock transport bytes and controlled or production media gates; execution 0018 separately validates an embedded real H.264 MP4 with production `FFprobeMediaProbe`, while executions 0019–0021 qualify bounded JPEG/PNG/WebP structures and reject tested GIF/APNG/animated WebP/AVIF payloads. The Tieba command now covers both compatible one-image and exact-two-image ARTICLE rows. The static gate is not a complete pixel decoder. These commands prove local pipeline contracts, not a real creator/feed/detail request, CDN/platform bytes or a running Emby/Jellyfin server. No retained real Zhihu or Tieba source fixture is claimed. / 第二至第九条命令分别是执行 0013 的 Bilibili、执行 0014 的快手、执行 0015 的抖音、执行 0016 的微博、执行 0017 的小红书静态内容、执行 0018 的小红书可播放视频、执行 0019 的知乎回答图片及执行 0020–0021 的贴吧首楼图片组合测试。它们使用合成元数据、fake detail 结果、mock transport 字节及受控或生产媒体门；执行 0018 另以生产 `FFprobeMediaProbe` 验证内嵌真实 H.264 MP4，执行 0019–0021 则校验有界 JPEG/PNG/WebP 结构并拒绝已测试的 GIF/APNG/animated WebP/AVIF 字节。贴吧命令现同时覆盖兼容单图与精确双图 ARTICLE 行。静态门不是完整像素解码器。这些命令只证明本地流水线契约，不证明真实 creator/feed/detail 请求、CDN/平台字节或运行中的 Emby/Jellyfin 服务器，也不宣称存在保留的真实知乎或贴吧来源夹具。
...
MediaCrawler-discovered assets persist only the stable, secret-free `adapter_refresh` locator. Execution 0009 resolves it lazily from the exact current Subscription source when `asset download` or the pipeline receives both MediaCrawler enable/license switches; manual selection also accepts `--subscription-id`. For XHS, executions 0017–0018 use the selected Subscription creator reference by default and retain the explicit one-note reference as an override. For Zhihu and Tieba, executions 0019–0021 accept no operator detail override: they derive the exact non-secret canonical answer/thread URL from the selected persisted ARTICLE/content row. Tieba exact-two-image refresh additionally binds both ordered persisted hints before resolving either position. Before any child Job or Asset lifecycle write, platform authority preflight binds the exact source and authority; the pipeline's existing capability gate separately validates the pinned lock/checkout/Python runtime plus an actually launchable mandatory `ffprobe` when required. Invalid authority fails with zero child lifecycle side effects. If an already-verified XHS archive is missing or invalid, XHS authority preflight also runs before quarantine or durable repair reset. Refreshed signed media results remain in memory and are never written back to SQLite.

MediaCrawler 发现的资产只持久化稳定且不含密钥的 `adapter_refresh` locator。执行 0009 会在 `asset download` 或 pipeline 同时收到 MediaCrawler 启用/许可证开关时，从精确当前 Subscription 来源惰性解析；手工选择另接受 `--subscription-id`。对小红书，执行 0017–0018 默认使用所选 Subscription 的作者引用，并保留显式单 note 引用作为覆盖。对知乎与贴吧，执行 0019–0021 不接受操作员 detail 覆盖，而是从选中持久 ARTICLE/content 行派生精确、非密钥的 canonical 回答/主题 URL；贴吧精确双图刷新还会在解析任一 position 前绑定两个有序持久 hint。在任何 child Job 或 Asset 生命周期写入前，平台权限 preflight 会绑定精确来源与权限；pipeline 既有 capability gate 会另行验证锁定 lock/checkout/Python runtime，以及需要时实际可启动的强制 `ffprobe`。无效权限以零 child 生命周期副作用失败。若已验证的小红书归档缺失或无效，也会在隔离或持久修复重置前运行小红书权限 preflight。刷新的签名媒体结果只在内存中使用，绝不写回 SQLite。

Offline refresh shapes are limited to XHS image/video, Douyin image/video/audio/cover, Kuaishou ordinary single video plus optional cover under an exact-one-play-URL boundary, Bilibili cover and logical-first-page single-progressive video, ordinary-original numeric-ID Weibo static IMAGE/GALLERY, one ordinary Zhihu answer with exactly one static IMAGE, and compatible one- or exact-two-static-image ordinary Tieba thread first floors. Composed media-library evidence is narrower: four frozen video slices, the Weibo two-image static gallery, the XHS ordinary `type="normal"` static gallery, the XHS ordinary `type="video"` VIDEO/MIXED shape, the Zhihu single-answer-image ARTICLE, and Tieba single- and exact-two-first-floor-image ARTICLE rows. Execution 0021 passes its 413-test focused regression, 1668-test complete suite and all quality/build/upstream/audit gates in implementation `e0fb8d5`. Tieba three-or-more-image galleries and other rich/media content; Zhihu multiple images, articles and zvideo; XHS multi-video/multi-image/broader mixed/live-photo/animation; Weibo video/GIF/long-image/effective `page_info`; Douyin galleries/music/multiple URLs; Kuaishou galleries/multiple URLs; and Bilibili DASH/mux/FLV/multiple pages/subtitles/danmaku/backup/bangumi/paid/live media are not implemented or qualified. Stable-identity replay cannot automatically detect same-origin/path byte replacement for the documented query-rotation cases. No real login, creator/feed/detail request, signed CDN/proxy download or Emby/Jellyfin rescan/playback has run for any platform; all such rows remain `NOT_RUN`.

离线刷新形状仅限小红书 image/video、抖音 image/video/audio/cover、“精确一个播放 URL”边界下的快手普通单视频与可选封面、Bilibili cover 与逻辑首 P 单 progressive 视频、普通原创 numeric-ID 微博静态 IMAGE/GALLERY、知乎普通回答精确一张静态 IMAGE，以及兼容单图或精确双静态图的贴吧普通主题首楼。组合媒体库证据更窄：四个冻结视频切片、微博普通原创双图静态 gallery、小红书普通 `type="normal"` 静态 gallery、小红书普通 `type="video"` VIDEO/MIXED 形状、知乎单回答图片 ARTICLE，以及贴吧单图与精确双图首楼 ARTICLE。Execution 0021 在实现 `e0fb8d5` 中通过 413 项专项回归、1668 项完整套件及全部质量/构建/上游/审计门。贴吧三张及以上 gallery/其他富内容或媒体类型，知乎多图/文章/zvideo，小红书多视频/多图片/更广混合/实况/动图，微博视频/GIF/长图/有效 `page_info`，抖音图集/音乐/多媒体 URL，快手图集/多 URL，以及 Bilibili DASH/mux/FLV/多 P/字幕/弹幕/备用地址/番剧/付费/直播媒体尚未实现或验收。文档所列 query 轮换场景在稳定身份不变时仍无法自动发现同 origin/path 字节替换。任何平台都尚未运行真人登录、creator/feed/detail 请求、签名 CDN/代理下载或 Emby/Jellyfin 重扫/播放；这些行全部保持 `NOT_RUN`。
