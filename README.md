...
The local function-first path is implemented through execution 0023 implementation commit `24fd41c`. In addition to explicit QR/session recovery and the foreground `scheduler supervise` chain, eleven frozen media shapes across all seven platforms now have focused offline evidence: execution 0013's Bilibili logical-first-page single-progressive video, execution 0014's Kuaishou ordinary single video plus optional cover, execution 0015's ordinary numeric-ID Douyin single video plus optional cover with empty image/music fields, execution 0016's ordinary-original Weibo static IMAGE/GALLERY, execution 0017's ordinary `type="normal"` XHS static IMAGE/GALLERY, execution 0018's ordinary `type="video"` XHS single playable video with zero or one static IMAGE artwork, execution 0019's ordinary Zhihu creator answer with exactly one static IMAGE, execution 0020's ordinary Tieba creator thread first floor with exactly one static IMAGE, execution 0021's compatible exact-two-static-image Tieba first floor, execution 0022's bounded 3–64-static-image Tieba first floor, and execution 0023's compatible 2–64-page Bilibili multipart progressive upload.

Execution 0023 adds a verified forward shim that captures canonical 1–64 `page`/`cid` identities before the pinned Bilibili store discards them. Exact-one-page `<aid>:video:0` remains compatible; qualifying 2–64-page uploads emit ordered locator-only `<aid>:video:cid:<cid>` Assets. Strict detail protocol v4 requests only the target CID and lazy refresh binds the complete persisted VIDEO sibling tuple, so missing, added, reordered, replaced, duplicated or malformed pages fail closed before URL return. Private page/play fields and signed URLs are recursively removed before persistence. The three-page composition proves three targeted detail calls, fixed Bilibili request profiles, distinct downloads and SHA-256 archives, deterministic Emby primary/two-part/NFO/source output and query-only replay with zero new detail/DNS/HTTP/probe/archive/export work. The focused regression passes `436 passed in 53.96s`, the complete suite passes `1739 passed, 1 skipped in 321.25s`, and all quality/build/docs/upstream/audit gates pass. Implementation `24fd41c` is pushed and reconciled. No retained real Bilibili fixture is claimed; real login, creator/detail/play APIs, CDN bytes and Emby/Jellyfin server validation remain `NOT_RUN`. Exact evidence is in [`docs/executions/0023-bilibili-bounded-multipart-progressive/verification.md`](docs/executions/0023-bilibili-bounded-multipart-progressive/verification.md).

本地功能优先链路已实现到执行 0023 实现提交 `24fd41c`。除显式 QR/会话回收及前台 `scheduler supervise` 全链外，覆盖全部七个平台的十一个冻结媒体形状现已有专项离线证据：执行 0013 的 Bilibili 逻辑首 P 单 progressive 视频、执行 0014 的快手普通单视频与可选封面、执行 0015 的普通 numeric-ID 抖音单视频与可选封面（图片与音乐字段为空）、执行 0016 的普通原创微博静态 IMAGE/GALLERY、执行 0017 的普通 `type="normal"` 小红书静态 IMAGE/GALLERY、执行 0018 的普通 `type="video"` 小红书单个可播放视频与零或一张静态 IMAGE 封面、执行 0019 的知乎作者普通回答精确一张静态 IMAGE、执行 0020 的贴吧作者普通主题首楼精确一张静态 IMAGE、执行 0021 兼容扩展的贴吧首楼精确两张静态 IMAGE、执行 0022 的贴吧首楼 3–64 张有界静态 IMAGE，以及执行 0023 兼容扩展的 Bilibili 2–64 分 P progressive 投稿。

执行 0023 增加经过校验的 forward shim，在锁定 Bilibili store 丢弃数据前捕获规范 1–64 项 `page`/`cid` 身份。精确单 P `<aid>:video:0` 保持兼容；合格 2–64 分 P 投稿产生有序、仅 locator 的 `<aid>:video:cid:<cid>` Asset。严格详情协议 v4 只请求目标 CID，惰性刷新绑定完整持久 VIDEO 兄弟元组，因此缺失、新增、重排、替换、重复或畸形分 P 均在返回 URL 前关闭失败；私有分 P/播放字段与签名 URL 会在持久化前递归移除。三分 P 组合证明三次定向详情调用、固定 Bilibili 请求 profile、不同下载与 SHA-256 归档、确定性 Emby 主媒体/两个 part/NFO/source 输出，以及仅 query 变化的重放零新增 detail/DNS/HTTP/probe/archive/export 工作。专项回归通过 `436 passed in 53.96s`，完整套件通过 `1739 passed, 1 skipped in 321.25s`，全部质量/构建/文档/上游/审计门通过。实现 `24fd41c` 已推送并核对。不宣称保留了真实 Bilibili 夹具；真人登录、creator/detail/play API、CDN 字节及 Emby/Jellyfin 服务器验收保持 `NOT_RUN`。准确证据位于 [`docs/executions/0023-bilibili-bounded-multipart-progressive/verification.md`](docs/executions/0023-bilibili-bounded-multipart-progressive/verification.md)。

- Implemented with focused offline evidence / 已实现并有专项离线证据：the eleven frozen shapes above, including compatible Bilibili single-/2–64-page progressive publication, bounded Zhihu/Tieba discovery and exact canonical refresh/archive/Emby output / 上述十一个冻结形状，包括兼容 Bilibili 单 P/2–64 分 P progressive 发布、知乎/贴吧有界发现及精确 canonical 刷新/归档/Emby 输出。
- Still pending or unclaimed / 仍待实现或未验收：Bilibili DASH/mux, segmented `durl`, subtitles/danmaku, backup/FLV and pages above 64; Tieba galleries above 64 and mixed/rich/reply media; Zhihu multiple images/articles/zvideo; other shapes in the larger seven-platform goal; and every live platform/CDN/real-byte/media-server row / Bilibili DASH/mux、分段 `durl`、字幕/弹幕、备用地址/FLV 与超过 64 个分 P；贴吧超过 64 张的 gallery 与混合/富内容/回复媒体；知乎多图/文章/zvideo；七平台大目标中的其他形状；以及全部真人平台/CDN/真实字节/媒体服务器行。
...
The second through tenth commands are the execution 0013 single-page and execution 0023 multipart Bilibili compositions, execution 0014 Kuaishou, execution 0015 Douyin, execution 0016 Weibo, execution 0017 XHS static, execution 0018 XHS playable-video, execution 0019 Zhihu answer-image and executions 0020–0022 Tieba first-floor-image compositions. They use synthetic metadata, fake detail results, mock transport bytes and controlled or production media gates. Execution 0023 additionally proves three target-CID calls, Bilibili request profiles, primary/two-part Emby layout and zero-work replay; execution 0018 separately validates an embedded real H.264 MP4 with production `FFprobeMediaProbe`, while executions 0019–0022 qualify bounded JPEG/PNG/WebP structures and reject tested GIF/APNG/animated WebP/AVIF payloads. The Tieba command covers compatible one-image, exact-two-image and v3 three-image ARTICLE rows, with separate unit/contract coverage at the 64/65 boundary. The static gate is not a complete pixel decoder. These commands prove local pipeline contracts, not a real creator/feed/detail request, CDN/platform bytes or a running Emby/Jellyfin server. No retained real Bilibili, Zhihu or Tieba source fixture is claimed. / 第二至第十条命令分别是执行 0013 的 Bilibili 单 P、执行 0023 的 Bilibili 多分 P、执行 0014 的快手、执行 0015 的抖音、执行 0016 的微博、执行 0017 的小红书静态内容、执行 0018 的小红书可播放视频、执行 0019 的知乎回答图片及执行 0020–0022 的贴吧首楼图片组合测试。它们使用合成元数据、fake detail 结果、mock transport 字节及受控或生产媒体门。执行 0023 另证明三次目标 CID 调用、Bilibili 请求 profile、Emby 主媒体/两个 part 布局与零工作重放；执行 0018 另以生产 `FFprobeMediaProbe` 验证内嵌真实 H.264 MP4，执行 0019–0022 则校验有界 JPEG/PNG/WebP 结构并拒绝已测试的 GIF/APNG/animated WebP/AVIF 字节。贴吧命令覆盖兼容单图、精确双图与 v3 三图 ARTICLE 行，另有单元/合约覆盖 64/65 边界。静态门不是完整像素解码器。这些命令只证明本地流水线契约，不证明真实 creator/feed/detail 请求、CDN/平台字节或运行中的 Emby/Jellyfin 服务器，也不宣称存在保留的真实 Bilibili、知乎或贴吧来源夹具。
...
MediaCrawler-discovered assets persist only the stable, secret-free `adapter_refresh` locator. Execution 0009 resolves it lazily from the exact current Subscription source when `asset download` or the pipeline receives both MediaCrawler enable/license switches; manual selection also accepts `--subscription-id`. For XHS, executions 0017–0018 use the selected Subscription creator reference by default and retain the explicit one-note reference as an override. For Zhihu and Tieba, executions 0019–0022 accept no operator detail override: they derive the exact non-secret canonical answer/thread URL from the selected persisted ARTICLE/content row. Tieba refresh binds the complete ordered persisted sibling tuple of 1–64 image hints before resolving any position. Execution 0023 similarly binds every Bilibili VIDEO refresh to the complete 1–64 persisted sibling tuple and sends only the target CID to detail protocol v4. Before any child Job or Asset lifecycle write, platform authority preflight binds the exact source and authority; the pipeline's existing capability gate separately validates the pinned lock/checkout/Python runtime plus an actually launchable mandatory `ffprobe` when required. Invalid authority fails with zero child lifecycle side effects. If an already-verified XHS archive is missing or invalid, XHS authority preflight also runs before quarantine or durable repair reset. Refreshed signed media results remain in memory and are never written back to SQLite.

MediaCrawler 发现的资产只持久化稳定且不含密钥的 `adapter_refresh` locator。执行 0009 会在 `asset download` 或 pipeline 同时收到 MediaCrawler 启用/许可证开关时，从精确当前 Subscription 来源惰性解析；手工选择另接受 `--subscription-id`。对小红书，执行 0017–0018 默认使用所选 Subscription 的作者引用，并保留显式单 note 引用作为覆盖。对知乎与贴吧，执行 0019–0022 不接受操作员 detail 覆盖，而是从选中持久 ARTICLE/content 行派生精确、非密钥的 canonical 回答/主题 URL；贴吧刷新会在解析任一 position 前绑定完整的 1–64 项有序兄弟图片 hint 元组。执行 0023 同样把每个 Bilibili VIDEO 刷新绑定到完整 1–64 项持久兄弟元组，并只把目标 CID 发送给详情协议 v4。在任何 child Job 或 Asset 生命周期写入前，平台权限 preflight 会绑定精确来源与权限；pipeline 既有 capability gate 会另行验证锁定 lock/checkout/Python runtime，以及需要时实际可启动的强制 `ffprobe`。无效权限以零 child 生命周期副作用失败。若已验证的小红书归档缺失或无效，也会在隔离或持久修复重置前运行小红书权限 preflight。刷新的签名媒体结果只在内存中使用，绝不写回 SQLite。

Offline refresh shapes are limited to XHS image/video, Douyin image/video/audio/cover, Kuaishou ordinary single video plus optional cover under an exact-one-play-URL boundary, Bilibili cover plus compatible single-page or 2–64-page single-progressive-per-page ordinary uploads, ordinary-original numeric-ID Weibo static IMAGE/GALLERY, one ordinary Zhihu answer with exactly one static IMAGE, and compatible one-image, exact-two-image or 3–64-static-image ordinary Tieba thread first floors. Composed media-library evidence is narrower: five frozen video slices, the Weibo two-image static gallery, the XHS ordinary `type="normal"` static gallery, the XHS ordinary `type="video"` VIDEO/MIXED shape, the Zhihu single-answer-image ARTICLE, and Tieba one-, two- and three-first-floor-image ARTICLE rows. Execution 0023 passes its 436-test focused regression, 1739-test complete suite and all quality/build/docs/upstream/audit gates in implementation `24fd41c`. Tieba galleries above 64 images and mixed/rich/reply media; Zhihu multiple images, articles and zvideo; XHS multi-video/multi-image/broader mixed/live-photo/animation; Weibo video/GIF/long-image/effective `page_info`; Douyin galleries/music/multiple URLs; Kuaishou galleries/multiple URLs; and Bilibili DASH/mux/FLV/multiple `durl` segments/pages above 64/subtitles/danmaku/backup/bangumi/paid/live media are not implemented or qualified. Stable-identity replay cannot automatically detect same-origin/path byte replacement for the documented query-rotation cases. No real login, creator/feed/detail request, signed CDN/proxy download or Emby/Jellyfin rescan/playback has run for any platform; all such rows remain `NOT_RUN`.

离线刷新形状仅限小红书 image/video、抖音 image/video/audio/cover、“精确一个播放 URL”边界下的快手普通单视频与可选封面、Bilibili cover 加兼容单 P 或 2–64 分 P 且每 P 单 progressive 的普通投稿、普通原创 numeric-ID 微博静态 IMAGE/GALLERY、知乎普通回答精确一张静态 IMAGE，以及兼容单图、精确双图或 3–64 张静态图的贴吧普通主题首楼。组合媒体库证据更窄：五个冻结视频切片、微博普通原创双图静态 gallery、小红书普通 `type="normal"` 静态 gallery、小红书普通 `type="video"` VIDEO/MIXED 形状、知乎单回答图片 ARTICLE，以及贴吧一图、二图与三图首楼 ARTICLE。Execution 0023 在实现 `24fd41c` 中通过 436 项专项回归、1739 项完整套件及全部质量/构建/文档/上游/审计门。贴吧超过 64 张的 gallery 与混合/富内容/回复媒体，知乎多图/文章/zvideo，小红书多视频/多图片/更广混合/实况/动图，微博视频/GIF/长图/有效 `page_info`，抖音图集/音乐/多媒体 URL，快手图集/多 URL，以及 Bilibili DASH/mux/FLV/多 `durl` 分段/超过 64 个分 P/字幕/弹幕/备用地址/番剧/付费/直播媒体尚未实现或验收。文档所列 query 轮换场景在稳定身份不变时仍无法自动发现同 origin/path 字节替换。任何平台都尚未运行真人登录、creator/feed/detail 请求、签名 CDN/代理下载或 Emby/Jellyfin 重扫/播放；这些行全部保持 `NOT_RUN`。
