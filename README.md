...
The local function-first path is implemented through execution 0018 implementation commit `356e254`, following plan commit `c9d3586`. In addition to explicit QR/session recovery and the foreground `scheduler supervise` chain, six frozen media shapes across five platforms now reach qualified offline output: execution 0013's Bilibili logical-first-page single-progressive video, execution 0014's Kuaishou ordinary single video plus optional cover, execution 0015's ordinary numeric-ID Douyin single video plus optional cover with empty image/music fields, execution 0016's ordinary-original Weibo static IMAGE/GALLERY, execution 0017's ordinary `type="normal"` XHS static IMAGE/GALLERY, and execution 0018's ordinary `type="video"` XHS single playable video with zero or one static IMAGE artwork.

Execution 0018 reuses the selected Asset's exact `AssetRefreshSource → Subscription → Account` authority path. Automatic creator fallback now accepts only one matching raw `type="video"` row whose ordinary scalar `video_url` contains exactly one candidate and whose ordinary scalar `image_list` contains zero or one candidate; the normalized row must map one-to-one to exactly one position-zero VIDEO and optional IMAGE, yielding VIDEO or the narrow qualified MIXED shape. The initial locator must be HTTP/HTTPS on `xhscdn.com` or a valid subdomain, with a non-root path and no userinfo, fragment, malformed label or custom port; redirects remain governed by the existing per-hop public-network checks. DEFAULT-profile mock HTTP, a real embedded H.264 file through production `FFprobeMediaProbe`, deterministic recording-probe composition, immutable SHA-256 archives and idempotent Emby `.mp4`/poster/NFO/source output all pass. Authority and signed media queries remain ephemeral and query-only replay adds no detail, DNS, HTTP, probe, archive or export work. Multi-video, multi-image, broader mixed-media, live-photo and animation shapes remain deferred. No coverage run is claimed; real login, creator/feed/detail requests, CDN/platform bytes and Emby/Jellyfin server validation all remain `NOT_RUN`. This is not seven-platform complete download, an auto-restarting service, forced synchronous-thread cancellation or cross-host HA. Exact evidence is in [`docs/executions/0018-xhs-playable-video/verification.md`](docs/executions/0018-xhs-playable-video/verification.md).

本地功能优先链路已实现到执行 0018 实现提交 `356e254`，其计划提交为 `c9d3586`。除显式 QR/会话回收及前台 `scheduler supervise` 全链外，覆盖五个平台的六个冻结媒体形状现已产生通过验收的离线输出：执行 0013 的 Bilibili 逻辑首 P 单 progressive 视频、执行 0014 的快手普通单视频与可选封面、执行 0015 的普通 numeric-ID 抖音单视频与可选封面（图片与音乐字段为空）、执行 0016 的普通原创微博静态 IMAGE/GALLERY、执行 0017 的普通 `type="normal"` 小红书静态 IMAGE/GALLERY，以及执行 0018 的普通 `type="video"` 小红书单个可播放视频与零或一张静态 IMAGE 封面。

执行 0018 复用所选 Asset 的精确 `AssetRefreshSource → Subscription → Account` 权限路径。自动 creator fallback 只接受唯一匹配的 raw `type="video"` 行：普通标量 `video_url` 精确包含一个候选，普通标量 `image_list` 包含零或一个候选；归一化结果必须一一映射为 position 0 的唯一 VIDEO 与可选 IMAGE，形成 VIDEO 或本次窄范围验收的 MIXED。初始 locator 必须是 `xhscdn.com` 或合法子域上的 HTTP/HTTPS 非根路径，不得包含 userinfo、fragment、畸形 label 或自定义端口；重定向继续受既有逐跳公网检查约束。DEFAULT-profile mock HTTP、内嵌真实 H.264 文件通过生产 `FFprobeMediaProbe`、确定性记录型 probe 组合、不可变 SHA-256 归档及幂等 Emby `.mp4`/poster/NFO/source 输出均已通过。权限与签名媒体 query 保持瞬态，仅 query 变化的重放不新增 detail、DNS、HTTP、probe、归档或导出工作。多视频、多图片、更广混合媒体、实况照片及动图继续延期。不宣称运行过 coverage；真人登录、creator/feed/detail 请求、CDN/平台字节及 Emby/Jellyfin 服务器验收全部保持 `NOT_RUN`。这不代表七平台完整下载，也不是自动重启服务，不提供同步线程强停或跨主机 HA。准确证据位于 [`docs/executions/0018-xhs-playable-video/verification.md`](docs/executions/0018-xhs-playable-video/verification.md)。

- Implemented offline / 已实现离线：the six frozen shapes above, including automatic XHS creator-authority lookup for ordinary static IMAGE/GALLERY and the narrow ordinary single-video VIDEO/MIXED shape through archive and Emby filesystem publication / 上述六个冻结形状，包括小红书普通静态 IMAGE/GALLERY 及窄范围普通单视频 VIDEO/MIXED 的自动作者权限查找、归档与 Emby 文件系统发布。
- Still pending or unclaimed / 仍待实现或未验收：other platform/media shapes across the larger seven-platform goal, including XHS multi-video/multi-image/broader-mixed/live-photo/animation, Weibo video/retweets/effective `page_info`/animation, stronger creator pagination and expiry recovery, and live platform/CDN/real-byte/media-server qualification / 七平台大目标中的其他平台与媒体形状，包括小红书多视频/多图片/更广混合/实况照片/动图、微博视频/转发/有效 `page_info`/动图、更强的作者分页与权限过期恢复，以及真人平台/CDN/真实字节/媒体服务器验收。
...
The second through seventh commands are the execution 0013 Bilibili, execution 0014 Kuaishou, execution 0015 Douyin, execution 0016 Weibo, execution 0017 XHS static and execution 0018 XHS playable-video compositions. They use synthetic metadata, fake detail results, mock transport bytes and controlled media validation; execution 0018 separately validates an embedded real H.264 MP4 with production `FFprobeMediaProbe`, while its deterministic composition uses a recording probe. They prove local pipeline contracts, not a real creator/feed/detail request, CDN/platform bytes or a running Emby/Jellyfin server. / 第二至第七条命令分别是执行 0013 的 Bilibili、执行 0014 的快手、执行 0015 的抖音、执行 0016 的微博、执行 0017 的小红书静态内容及执行 0018 的小红书可播放视频组合测试。它们使用合成元数据、fake detail 结果、mock transport 字节及受控媒体校验；执行 0018 另以生产 `FFprobeMediaProbe` 验证内嵌真实 H.264 MP4，而确定性组合使用记录型 probe。它们只证明本地流水线契约，不证明真实 creator/feed/detail 请求、CDN/平台字节或运行中的 Emby/Jellyfin 服务器。
...
MediaCrawler-discovered assets persist only the stable, secret-free `adapter_refresh` locator. Execution 0009 resolves it lazily from the exact current Subscription source when `asset download` or the pipeline receives both MediaCrawler enable/license switches; manual selection also accepts `--subscription-id`. For XHS, executions 0017–0018 use the selected Subscription creator reference by default and retain the explicit one-note reference as an override. Before any child Job or Asset lifecycle write, XHS authority preflight binds the exact source and authority; the pipeline's existing capability gate separately validates the pinned lock/checkout/Python runtime plus an actually launchable mandatory `ffprobe` when required. Invalid authority fails with zero child lifecycle side effects. If an already-verified XHS archive is missing or invalid, authority preflight also runs before quarantine or durable repair reset. The signed result is used in memory by the safe downloader and is never written back to SQLite.

MediaCrawler 发现的资产只持久化稳定且不含密钥的 `adapter_refresh` locator。执行 0009 会在 `asset download` 或 pipeline 同时收到 MediaCrawler 启用/许可证开关时，从精确当前 Subscription 来源惰性解析；手工选择另接受 `--subscription-id`。对小红书，执行 0017–0018 默认使用所选 Subscription 的作者引用，并保留显式单 note 引用作为覆盖。在任何 child Job 或 Asset 生命周期写入前，小红书权限 preflight 会绑定精确来源与权限；pipeline 既有 capability gate 会另行验证锁定 lock/checkout/Python runtime，以及需要时实际可启动的强制 `ffprobe`。无效权限以零 child 生命周期副作用失败。若已验证的小红书归档缺失或无效，也会在隔离或持久修复重置前运行权限 preflight。签名结果只在安全下载器内存中使用，绝不写回 SQLite。

Offline refresh shapes are limited to XHS image/video, Douyin image/video/audio/cover, Kuaishou ordinary single video plus optional cover under an exact-one-play-URL boundary, Bilibili cover and logical-first-page single-progressive video, plus ordinary-original numeric-ID Weibo static IMAGE/GALLERY. Composed media-library qualification is narrower: four frozen video slices, the Weibo two-image static gallery, the XHS ordinary `type="normal"` static gallery and the XHS ordinary `type="video"` shape with exactly one position-zero VIDEO plus zero or one static IMAGE. The latter is VIDEO when artwork is absent and the narrow qualified MIXED shape when one artwork IMAGE is present. Multi-video, multi-image, broader mixed-media, live-photo and animation XHS shapes remain unqualified; the explicit-note video path remains a compatibility path outside the automatic creator claim. Tieba and Zhihu currently have no normalized downloadable Asset. Weibo video, GIF/animation, long-image-specific handling and effective `page_info`; Douyin galleries, associated-music semantics and multiple media URLs; Kuaishou galleries/multiple URLs; and Bilibili DASH/mux, FLV remux, multiple segments/pages, subtitles, danmaku, backup failover and bangumi/paid/live media are not implemented or not qualified. Query-only Douyin/Kuaishou/XHS rotation, same-aid Bilibili replay and same-ID Weibo replay cannot automatically detect byte replacement under the same stable identity. No real login, creator/feed/detail request, signed CDN/proxy download or Emby/Jellyfin rescan/playback has run for any platform; all such rows remain `NOT_RUN`.

离线刷新形状仅限小红书 image/video、抖音 image/video/audio/cover、“精确一个播放 URL”边界下的快手普通单视频与可选封面、Bilibili cover 与逻辑首 P 单 progressive 视频，以及普通原创 numeric-ID 微博静态 IMAGE/GALLERY。组合媒体库验收范围更窄：四个冻结视频切片、微博普通原创双图静态 gallery、小红书普通 `type="normal"` 静态 gallery，以及精确包含 position 0 唯一 VIDEO 与零或一个静态 IMAGE 的小红书普通 `type="video"` 形状。无封面时为 VIDEO，存在一张封面时为本次窄范围验收的 MIXED。小红书多视频、多图片、更广混合媒体、实况照片及动图仍未验收；显式 note 视频路径继续作为自动 creator 声明之外的兼容路径。贴吧与知乎当前没有已归一化的可下载 Asset。微博视频、GIF/动图、长图专用处理与有效 `page_info`，抖音图集、关联音乐语义与多媒体 URL，快手图集/多 URL，以及 Bilibili DASH/mux、FLV remux、多段/多 P、字幕、弹幕、备用地址故障切换及番剧/付费/直播媒体尚未实现或未验收。仅 query 变化的抖音/快手/小红书轮换、同 aid Bilibili 重放及同 ID 微博重放都无法在稳定身份不变时自动检测字节替换。任何平台都尚未运行真人登录、creator/feed/detail 请求、签名 CDN/代理下载或 Emby/Jellyfin 重扫/播放；这些行全部保持 `NOT_RUN`。
