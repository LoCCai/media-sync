...
The local function-first path is implemented through execution 0015 commit `95d314d`. In addition to explicit QR/session recovery and the foreground `scheduler supervise` chain, three frozen platform shapes now reach playable offline output: execution 0013's Bilibili logical-first-page single-progressive video, execution 0014's Kuaishou ordinary single video plus optional cover, and execution 0015's ordinary numeric-ID Douyin single video plus optional cover with empty image/music fields. The Douyin slice proves pure-ID detail in an isolated fake checkout, exact Account/Subscription refresh, transient signed URLs, default-profile MP4/PNG transfer, controlled probe/archive and Emby `.mp4`/poster publication with idempotent replay. The execution 0015 focused gate passes 231 tests; the complete suite passes 1209 tests with one Windows-inapplicable POSIX mode-bit test skipped. No coverage run is claimed. No real QR scan, platform account, creator endpoint, CDN or media server was used, so every live row remains `NOT_RUN`. This is not an auto-restarting daemon, Windows Service/systemd unit, forced synchronous-thread cancellation, cross-host HA or seven-platform complete download. Exact evidence is in [`docs/executions/0015-douyin-playable-video/verification.md`](docs/executions/0015-douyin-playable-video/verification.md).

本地功能优先链路已实现到执行 0015 提交 `95d314d`。除显式 QR/会话回收及前台 `scheduler supervise` 全链外，三个冻结的平台形状现已可产生离线可播放输出：执行 0013 的 Bilibili 逻辑首 P 单 progressive 视频、执行 0014 的快手普通单视频与可选封面，以及执行 0015 的普通 numeric-ID 抖音单视频与可选封面（图片与音乐字段为空）。抖音切片由隔离 fake checkout 证明纯 ID detail，并证明精确 Account/Subscription 刷新、瞬态签名 URL、默认 profile MP4/PNG 传输、受控探测/归档及幂等 Emby `.mp4`/海报发布。执行 0015 专项门禁通过 231 项；完整套件通过 1209 项，另有一项 Windows 不适用的 POSIX mode-bit 测试跳过。不宣称运行过覆盖率。没有使用真人二维码扫码、平台账户、作者端点、CDN 或媒体服务器，因此全部真人行保持 `NOT_RUN`。它不是自动重启 daemon、Windows Service/systemd 服务，不提供同步线程强停、跨主机 HA 或七平台完整下载。准确证据位于 [`docs/executions/0015-douyin-playable-video/verification.md`](docs/executions/0015-douyin-playable-video/verification.md)。
...
The second through fourth commands are the execution 0013 Bilibili, execution 0014 Kuaishou and execution 0015 Douyin composition tests. They use synthetic metadata, fake detail results, mock transport bytes and controlled probes; they prove the local pipeline contracts, not a real CDN, real FFmpeg against platform media or a running Emby/Jellyfin server. / 第二至第四条命令分别是执行 0013 的 Bilibili、执行 0014 的快手及执行 0015 的抖音组合测试。它们使用合成元数据、fake detail 结果、mock transport 字节及受控 probe；只证明本地流水线契约，不证明真实 CDN、针对平台媒体的真实 FFmpeg 或运行中的 Emby/Jellyfin 服务器。
...
Offline refresh shapes are limited to XHS image/video, Douyin image/video/audio/cover, Kuaishou ordinary single video plus optional cover under an exact-one-play-URL boundary, Bilibili cover, and one Bilibili ordinary-upload logical-first-page video only when its play response contains exactly one progressive `durl`. Composed playable qualification is narrower: Bilibili logical first-page single progressive, Kuaishou single video plus optional cover, and Douyin ordinary numeric-ID single video plus optional cover with empty note/music fields. Weibo, Tieba and Zhihu currently have no normalized downloadable Asset. Douyin galleries, associated-music semantics and multiple media URLs; Kuaishou galleries/multiple URLs; and Bilibili DASH/mux, FLV remux, multiple segments/pages, subtitles, danmaku, backup failover and bangumi/paid/live media are not implemented or not qualified. Query-only Douyin/Kuaishou rotation and same-aid Bilibili replay cannot automatically detect byte replacement under the same stable identity. No real login, creator request, signed CDN download or Emby/Jellyfin rescan/playback has run for any platform; all such rows remain `NOT_RUN`.

离线刷新形状仅限小红书 image/video、抖音 image/video/audio/cover、“精确一个播放 URL”边界下的快手普通单视频与可选封面、Bilibili cover，以及“播放响应精确包含一个 progressive `durl`”时的 Bilibili 普通投稿逻辑首 P 视频。组合可播放验收范围更窄：Bilibili 逻辑首 P 单 progressive、快手单视频与可选封面，以及 note/music 为空的抖音普通 numeric-ID 单视频与可选封面。微博、贴吧、知乎当前没有已归一化的可下载 Asset。抖音图集、关联音乐语义与多媒体 URL，快手图集/多 URL，以及 Bilibili DASH/mux、FLV remux、多段/多 P、字幕、弹幕、备用地址故障切换及番剧/付费/直播媒体尚未实现或未验收。仅 query 变化的抖音/快手轮换与同 aid Bilibili 重放都无法在稳定身份不变时自动检测字节替换。任何平台都尚未运行真人登录、作者请求、签名 CDN 下载或 Emby/Jellyfin 重扫/播放；这些行全部保持 `NOT_RUN`。
