...
| Dimension | State |
| --- | --- |
| Offline implementation | Commit `f19bfaa` delivers the backend single-operator credential/session/CSRF boundary and optional separate Bearer automation; `1d5b448` adds matched-only observation identity, revision `0008_playback_evidence`, and append-only create-or-replay persistence. The current checkpoint adds the TOCTOU-safe confirmation service and browser-only POST. It does **not** yet add the bounded by-author current/stale projection, qualification schema v3, confirmation UI, or Web authentication integration |
| Offline verification | The current tree passes **2941 tests with 22 skips and 1 existing warning**. The skips are 3 Windows/POSIX differences, 11 Operation PostgreSQL cases and 8 PlaybackEvidence PostgreSQL cases because this workstation has no test URL. The 69 Web tests plus format/check/build, Ruff/format across 731 files, strict mypy across 106 source files, compileall and the 498-file docs check pass. Real PostgreSQL and Docker were not run; the isolated PostgreSQL harness does not establish complete deployment support |
...
| Live qualification | **Every implemented live platform/CDN/media-server row remains `NOT_RUN`**. Until projection/schema v3 lands, the qualification API still reports overall playback evidence as `NOT_IMPLEMENTED`; provider task completion and automatic post-export scan are also `NOT_IMPLEMENTED` |
| Release blockers | Bounded playback-evidence projection and qualification v3, Web authentication integration and confirmation UI, the Linux operator baseline, and zero live rows; see [`docs/status.md`](docs/status.md) |
