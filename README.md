...
The local function-first path is implemented through execution 0014 commit `c4ab537`. In addition to explicit QR/session recovery and the foreground `scheduler supervise` chain, two frozen platform shapes now reach playable offline output: execution 0013's Bilibili logical-first-page single-progressive video and execution 0014's Kuaishou ordinary single video plus optional cover. The Kuaishou path proves pure-ID detail, exact Account/Subscription refresh, transient signed URLs, default-profile MP4/PNG transfer, controlled probe/archive and Emby `.mp4`/poster publication with idempotent replay. The execution 0014 focused gate passes 228 tests; the complete suite passes 1206 tests with one Windows-inapplicable POSIX mode-bit test skipped. No coverage run is claimed. No real QR scan, platform account, creator endpoint, CDN or media server was used, so every live row remains `NOT_RUN`. This is not an auto-restarting daemon, Windows Service/systemd unit, forced synchronous-thread cancellation, cross-host HA or seven-platform complete download. Exact evidence is in [`docs/executions/0014-kuaishou-playable-video/verification.md`](docs/executions/0014-kuaishou-playable-video/verification.md).

本地功能优先链路已实现到执行 0014 提交 `c4ab537`。除显式 QR/会话回收及前台 `scheduler supervise` 全链外，两个冻结的平台形状现已可产生离线可播放输出：执行 0013 的 Bilibili 逻辑首 P 单 progressive 视频，以及执行 0014 的快手普通单视频与可选封面。快手路径证明纯 ID detail、精确 Account/Subscription 刷新、瞬态签名 URL、默认 profile MP4/PNG 传输、受控探测/归档及幂等 Emby `.mp4`/海报发布。执行 0014 专项门禁通过 228 项；完整套件通过 1206 项，另有一项 Windows 不适用的 POSIX mode-bit 测试跳过。不宣称运行过覆盖率。没有使用真人二维码扫码、平台账户、作者端点、CDN 或媒体服务器，因此全部真人行保持 `NOT_RUN`。它不是自动重启 daemon、Windows Service/systemd 服务，不提供同步线程强停、跨主机 HA 或七平台完整下载。准确证据位于 [`docs/executions/0014-kuaishou-playable-video/verification.md`](docs/executions/0014-kuaishou-playable-video/verification.md)。
...
The second and third commands are the execution 0013 Bilibili and execution 0014 Kuaishou composition tests. They use synthetic metadata, fake detail results, mock transport bytes and controlled probes; they prove the local pipeline contracts, not a real CDN, real FFmpeg against platform media or a running Emby/Jellyfin server. / 第二、第三条命令分别是执行 0013 的 Bilibili 与执行 0014 的快手组合测试。它们使用合成元数据、fake detail 结果、mock transport 字节及受控 probe；只证明本地流水线契约，不证明真实 CDN、针对平台媒体的真实 FFmpeg 或运行中的 Emby/Jellyfin 服务器。
...
Offline refresh shapes are limited to XHS image/video, Douyin image/video/audio/cover, Kuaishou ordinary single video plus optional cover under an exact-one-play-URL boundary, Bilibili cover, and one Bilibili ordinary-upload logical-first-page video only when its play response contains exactly one progressive `durl`. Weibo, Tieba and Zhihu currently have no normalized downloadable Asset. Kuaishou galleries/multiple URLs and Bilibili DASH/mux, FLV remux, multiple segments/pages, subtitles, danmaku, backup failover and bangumi/paid/live media are not implemented. Query-only Kuaishou rotation and same-aid Bilibili replay cannot automatically detect byte replacement under the same stable identity. No real login, creator request, signed CDN download or Emby/Jellyfin rescan/playback has run for any platform; all such rows remain `NOT_RUN`.

离线刷新形状仅限小红书 image/video、抖音 image/video/audio/cover、“精确一个播放 URL”边界下的快手普通单视频与可选封面、Bilibili cover，以及“播放响应精确包含一个 progressive `durl`”时的 Bilibili 普通投稿逻辑首 P 视频。微博、贴吧、知乎当前没有已归一化的可下载 Asset。快手图集/多 URL，以及 Bilibili DASH/mux、FLV remux、多段/多 P、字幕、弹幕、备用地址故障切换及番剧/付费/直播媒体尚未实现。仅 query 变化的快手轮换与同 aid Bilibili 重放都无法在稳定身份不变时自动检测字节替换。任何平台都尚未运行真人登录、作者请求、签名 CDN 下载或 Emby/Jellyfin 重扫/播放；这些行全部保持 `NOT_RUN`。
