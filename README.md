...
The local function-first path is implemented through execution 0022 implementation commit `b6d03aa`. In addition to explicit QR/session recovery and the foreground `scheduler supervise` chain, ten frozen media shapes across all seven platforms now have focused offline evidence: execution 0013's Bilibili logical-first-page single-progressive video, execution 0014's Kuaishou ordinary single video plus optional cover, execution 0015's ordinary numeric-ID Douyin single video plus optional cover with empty image/music fields, execution 0016's ordinary-original Weibo static IMAGE/GALLERY, execution 0017's ordinary `type="normal"` XHS static IMAGE/GALLERY, execution 0018's ordinary `type="video"` XHS single playable video with zero or one static IMAGE artwork, execution 0019's ordinary Zhihu creator answer with exactly one static IMAGE, execution 0020's ordinary Tieba creator thread first floor with exactly one static IMAGE, execution 0021's compatible exact-two-static-image Tieba first floor, and execution 0022's bounded 3–64-static-image Tieba first floor.

Execution 0022 preserves execution 0020's v1 single-image and execution 0021's v2 exact-two-image fields, then adds a separate v3 claim for 3–64 ordered, distinct static images. Every item keeps the frozen ten-key/type-3/host/path/query contract; 65 or more images, multi-version claims, duplicate query-free identities and other first-floor content types fail closed. The ARTICLE emits `<note_id>:image:0..N-1`; SQLite retains only the complete ordered query-free identity tuple. Lazy refresh requires the current canonical detail to reproduce the same complete size, order, identities, positions and remote IDs before returning one newly validated signed URL under credential-free `MediaRequestProfile.DEFAULT`. The three-image composition proves exact JPEG/PNG/WebP downloads, three SHA-256 archives and deterministic Emby poster/backdrop/three-file-gallery/body/NFO/source output; query-only replay performs zero new detail/DNS/HTTP/archive/export work. The focused regression passes `433 passed in 48.91s`, the complete suite passes `1688 passed, 1 skipped in 321.22s`, and all quality/build/docs/upstream/audit gates pass. Implementation `b6d03aa` is pushed and reconciled. There is no retained real Tieba fixture; real login, creator/detail, future CDN bytes and Emby/Jellyfin server validation remain `NOT_RUN`. Exact evidence is in [`docs/executions/0022-tieba-bounded-static-gallery/verification.md`](docs/executions/0022-tieba-bounded-static-gallery/verification.md).

本地功能优先链路已实现到执行 0022 实现提交 `b6d03aa`。除显式 QR/会话回收及前台 `scheduler supervise` 全链外，覆盖全部七个平台的十个冻结媒体形状现已有专项离线证据：执行 0013 的 Bilibili 逻辑首 P 单 progressive 视频、执行 0014 的快手普通单视频与可选封面、执行 0015 的普通 numeric-ID 抖音单视频与可选封面（图片与音乐字段为空）、执行 0016 的普通原创微博静态 IMAGE/GALLERY、执行 0017 的普通 `type="normal"` 小红书静态 IMAGE/GALLERY、执行 0018 的普通 `type="video"` 小红书单个可播放视频与零或一张静态 IMAGE 封面、执行 0019 的知乎作者普通回答精确一张静态 IMAGE、执行 0020 的贴吧作者普通主题首楼精确一张静态 IMAGE、执行 0021 兼容扩展的贴吧首楼精确两张静态 IMAGE，以及执行 0022 的贴吧首楼 3–64 张有界静态 IMAGE。

执行 0022 保留执行 0020 的 v1 单图字段与执行 0021 的 v2 精确双图字段，并增加独立 v3 声明承载 3–64 张有序互异静态图片。每项继续满足冻结的十键/type-3/host/path/query 合约；65 张及以上、多版本声明、重复无 query 身份与其他首楼内容类型均关闭失败。ARTICLE 产生 `<note_id>:image:0..N-1`；SQLite 只保留完整有序无 query 身份元组。惰性刷新要求当前 canonical 详情在数量、顺序、身份、position 与 remote ID 上完整复现，才以无凭据 `MediaRequestProfile.DEFAULT` 返回一项新校验签名 URL。三图组合证明精确 JPEG/PNG/WebP 下载、三个 SHA-256 归档与确定性 Emby poster/backdrop/三文件 gallery/body/NFO/source 输出；仅 query 变化的重放不新增 detail/DNS/HTTP/archive/export 工作。专项回归通过 `433 passed in 48.91s`，完整套件通过 `1688 passed, 1 skipped in 321.22s`，全部质量/构建/文档/上游/审计门通过。实现 `b6d03aa` 已推送并核对。当前没有保留的真实贴吧夹具；真人登录、creator/detail、未来 CDN 字节及 Emby/Jellyfin 服务器验收保持 `NOT_RUN`。准确证据位于 [`docs/executions/0022-tieba-bounded-static-gallery/verification.md`](docs/executions/0022-tieba-bounded-static-gallery/verification.md)。

- Implemented with focused offline evidence / 已实现并有专项离线证据：the ten frozen shapes above, including bounded Zhihu-answer and Tieba-thread discovery, exact canonical refresh, and archive/Emby publication for compatible one-image, exact-two-image or 3–64-image Tieba first floors / 上述十个冻结形状，包括知乎回答与贴吧主题有界发现、精确 canonical 刷新，以及兼容单图、精确双图或 3–64 图贴吧首楼的归档/Emby 发布。
- Still pending or unclaimed / 仍待实现或未验收：Tieba galleries above 64 images and mixed/rich/reply media; Zhihu multiple images/articles/zvideo; other shapes in the larger seven-platform goal; and every live platform/CDN/real-byte/media-server row / 贴吧超过 64 张的 gallery 与混合/富内容/回复媒体；知乎多图/文章/zvideo；七平台大目标中的其他形状；以及全部真人平台/CDN/真实字节/媒体服务器行。
